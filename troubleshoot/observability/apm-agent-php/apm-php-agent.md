...
Is something not working as expected? Don’t worry if you can’t figure out what the problem is; we’re here to help! As a first step, ensure your app is compatible with the agent’s [supported technologies](apm-agent-php://docs/reference/ingestion-tools/apm-agent-php/supported-technologies.md).
...
Disable the agent by setting [`enabled`](apm-agent-php://docs/reference/ingestion-tools/apm-agent-php/configuration-reference.md#config-enabled) to `false`. You’ll need to restart your application for the changes to apply.
...
The log is written under `INFO` level - see [Logging](apm-agent-php://docs/reference/ingestion-tools/apm-agent-php/configuration.md#configure-logging).
