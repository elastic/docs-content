...
::::{warning}
Kibana’s `file` appender is configured to produce logs in [ECS JSON](ecs://docs/reference/ecs/index.md) format. It’s the only format that includes the meta information necessary for [log correlation](apm-agent-nodejs://docs/reference/ingestion-tools/apm-agent-nodejs/logs.md) out-of-the-box.
::::
...
**Prerequisites** {{kib}} logs are configured to be in [ECS JSON](ecs://docs/reference/ecs/index.md) format to include tracing identifiers.
