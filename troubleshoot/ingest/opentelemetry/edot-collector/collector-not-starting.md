...
* Confirm required components are defined
   ...
   * **For managed OTLP endpoint**: use relevant receivers and export using the `otlp` exporter
   ...
   Refer to [Default configuration of the EDOT Collector (Standalone)](opentelemetry:/elastic-agent://reference/edot-collector/config/default-config-standalone.md) for full examples for each use case.
