...
If {{agent}} was successfully enrolled, but doesn’t show up in the **Agents** list, it might not be started. Make sure the `elastic-agent` process is running on the host. If it’s not running, use the [`run`](docs-content://docs/reference/ingestion-tools/fleet/agent-command-reference.md#elastic-agent-run-command) command to start it.  The most common way to deploy an {{agent}} is by using the `install` command. This command starts the {{agent}} for you.
...
See [Installation layout](docs-content://docs/reference/ingestion-tools/fleet/installation-layout.md) to find out the exact paths for each platform.


## What policy is the {{agent}} running? [what-is-my-agent-config]

To find the policy file, inspect the `elastic-agent.yml` file in the directory where {{agent}} is running. Not sure where the agent is running? See [Installation layout](docs-content://docs/reference/ingestion-tools/fleet/installation-layout.md).
...
Although it’s redundant to install stand-alone {{metricbeat}}, you might want to try installing it to see if it’s able to send data successfully to {{es}}. For more information, see [{{metricbeat}} quick start](beats://docs/reference/ingestion-tools/beats-metricbeat/metricbeat-installation-configuration.md).
...
It’s okay, we’ve got your back! The data is still in {{es}}. To add {{agent}} to {{fleet}} again, [Stop {{agent}}](docs-content://docs/reference/ingestion-tools/fleet/start-stop-elastic-agent.md#stop-elastic-agent-service), re-enroll it on the host, then run {{agent}}.
...
* {{agent}} requires internet access to download artifacts for binary upgrades. In air-gapped environments, you can host your own artifact registry. For more information, refer to [Air-gapped environments](docs-content://docs/reference/ingestion-tools/fleet/air-gapped.md).
...
To reinstall the assets for a specific integration, you can use the {{fleet}} UI. For more information, see [Reinstall integration assets](docs-content://docs/reference/ingestion-tools/fleet/install-uninstall-integration-assets.md#reinstall-integration-assets).
