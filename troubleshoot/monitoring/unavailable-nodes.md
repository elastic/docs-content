...
::::{note}
To delete unused data API commands can be ran via Kibana [DevTools](https://www.elastic.co/docs/explore-analyze/query-filter/tools/console) only if it's available, but otherwise API commands will need to be ran via the Deployment's [Elasticsearch API Console](https://www.elastic.co/docs/reference/cloud/cloud-hosted/ec-api-console) which will be available as long as Elasticsearch has elected quorum.
::::
