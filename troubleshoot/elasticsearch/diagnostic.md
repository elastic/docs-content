...
2. Using the same environment parameters, run the diagnostic tool script.
    ...
    * `local` (default, recommended): Polls the [{{es}} API](elasticsearch://docs/reference/elasticsearch/rest-apis/index.md), gathers operating system info, and captures cluster and GC logs.
    * `remote`: Establishes an ssh session to the applicable target server to pull the same information as `local`.
    * `api`: Polls the [{{es}} API](elasticsearch://docs/reference/elasticsearch/rest-apis/index.md). All other data must be collected manually.
