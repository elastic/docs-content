...
* Reviewing the elected-master node logs will report a flood of INFO logs you see a large amount of `[elasticsearch.server][INFO] update_mapping [_doc]` messages. If they time out you will see a large amount of `org.elasticsearch.cluster.metadata.ProcessClusterEventTimeoutException: failed to process cluster event (put-mapping [<INDEX_NAME>/<INDEX_UUID>]) within 30s`.
