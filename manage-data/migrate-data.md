...
Depending on the type of data that you need to move, various migration options are available:
 ...
 - **Dual ingest**: For data with a limited lifecycle (logs and metrics, for example), another approach is to ingest into both the original and new environment at the same time, for a set duration. You can ingest into both environments for at least long enough for the data retention period to elapse. Then, after confirming that everything is working well in the new environment, the original environment can be shut down.
