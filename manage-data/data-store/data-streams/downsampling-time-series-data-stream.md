...
To downsample time series data as part of ILM, include a [Downsample action](elasticsearch://docs/reference/elasticsearch/index-lifecycle-actions/ilm-downsample.md) in your ILM policy and set `fixed_interval` to the level of granularity that you’d like:
...
* For [date histogram aggregations](elasticsearch://docs/reference/data-analysis/aggregations/search-aggregations-bucket-datehistogram-aggregation.md), only `fixed_intervals` (and not calendar-aware intervals) are supported.
* Timezone support comes with caveats:
    ...
    * Daylight savings and similar peculiarities around timezones affect reported results, as [documented](elasticsearch://docs/reference/data-analysis/aggregations/search-aggregations-bucket-datehistogram-aggregation.md#datehistogram-aggregation-time-zone) for date histogram aggregation. Besides, downsampling at daily interval hinders tracking any information related to daylight savings changes.
...
* Downsampling is provided as an ILM action. See [Downsample](elasticsearch://docs/reference/elasticsearch/index-lifecycle-actions/ilm-downsample.md).
...
* The numeric `gauge` and `counter` [metric types](elasticsearch://docs/reference/elasticsearch/mapping-reference/mapping-field-meta.md) are supported.
