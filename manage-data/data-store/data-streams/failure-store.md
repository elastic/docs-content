...
Note that a failure store is attached to a single data stream, if a [`reroute` processor](elasticsearch://reference/enrich-processor/reroute-processor.md) changes the target data stream of an indexing request then subsequent failures will also be redirected to the new target's failure store if enabled.
