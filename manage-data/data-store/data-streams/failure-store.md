...
A failure store is attached to a single data stream. If a [`reroute` processor](elasticsearch://reference/enrich-processor/reroute-processor.md) changes the target data stream of an indexing request, subsequent failures are redirected to the new target's failure store, if enabled.
