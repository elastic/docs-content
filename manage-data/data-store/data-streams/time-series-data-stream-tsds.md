...
* {applies_to}`stack: ga 9.4` **Sequence numbers are disabled:** A TSDS [disables sequence numbers](elasticsearch://reference/elasticsearch/index-settings/index-modules.md#index-disable-sequence-numbers) by default to substantially improve storage efficiency (up to 2x) and indexing throughput (10%). The downside is that [Optimistic Concurrency Control](elasticsearch://reference/elasticsearch/rest-apis/optimistic-concurrency-control.md) gets disabled and [update-by-query](elasticsearch://reference/elasticsearch/rest-apis/update-by-query-api.md) and [delete-by-query](elasticsearch://reference/elasticsearch/operation/operation-delete-by-query.md) operations execute with weaker consistency. These are normally not relevant for time series workloads, but if you need them for your application (and you are willing to pay the extra cost in storage and indexing resources), you can restore sequence numbers by setting `index.disable_sequence_numbers: false` in the index template of the involved TSDS.
