...
stack: ga 9.3
```
* **Docvalue Skippers:** A TSDS enables [docvalue skippers](elasticsearch://reference/elasticsearch/mapping-reference/doc-values.md#doc-values-skippers) on its `_tsid`, `@timestamp`, [dimension](#time-series-dimension) and [metric](#time-series-metric) fields. Because `tsid` and `@timestamp` are part of the index sort, this allows elasticsearch to avoid building backing indexes for these fields, meaning lower disk usage and faster ingest speed.
