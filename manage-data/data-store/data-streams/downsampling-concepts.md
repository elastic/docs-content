...
* `aggregate`
  ...
  * `counter` fields:
    * For [cumulative](/manage-data/data-store/data-streams/metric-temporality.md#cumulative-vs-delta) counters: stores the first value, and stores up to two data points if a counter reset is detected {applies_to}`stack: ga 9.4` {applies_to}`serverless: ga`. The field type is preserved. In previous versions, only the last value is stored.
    * {applies_to}`stack: ga 9.5` {applies_to}`serverless: ga` For [delta](/manage-data/data-store/data-streams/metric-temporality.md#cumulative-vs-delta) counters: stores the sum of values in the bucket.
  * `histogram` fields {applies_to}`stack: preview 9.3` {applies_to}`serverless: preview`
    ...
    * {applies_to}`stack: ga 9.5` {applies_to}`serverless: ga` The metric temporality is preserved. For [cumulative](/manage-data/data-store/data-streams/metric-temporality.md#cumulative-vs-delta) histograms, we use the same algorithm as for cumulative counters.
