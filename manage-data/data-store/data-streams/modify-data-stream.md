...
1. Update the index template used by the data stream. This ensures the updated field mapping is added to future backing indices created for the stream.
    ...
    The following [create or update index template](../templates.md) request changes the argument for the `host.ip` field’s [`ignore_malformed`](elasticsearch://docs/reference/elasticsearch/mapping-reference/ignore-malformed.md) mapping parameter to `true`.
...
To change a [dynamic index setting](elasticsearch://docs/reference/elasticsearch/index-settings/index.md) for a data stream, follow these steps:
