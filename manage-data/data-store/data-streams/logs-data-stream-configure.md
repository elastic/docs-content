...
If you have the required [subscription]({{subscriptions}}), `logsdb` index mode uses [synthetic `_source`](elasticsearch://reference/elasticsearch/mapping-reference/mapping-source-field.md#synthetic-source), which omits storing the original `_source` field. Instead, the document source is synthesized from doc values or stored fields upon document retrieval.

If you don't have the required [subscription]({{subscriptions}}), `logsdb` mode uses the original `_source` field.
...
If you have the required [subscription]({{subscriptions}}), you can enable routing optimizations to reduce the storage footprint of `logsdb` indexes. A routing optimization uses the fields in the sort configuration (except for `@timestamp`) to route documents to shards.
...
% TODO confirm sort.order for host.name
...
If your deployment uses runtime fields within rules for {{elastic-sec}}, you might encounter compatibility issues when using `logsdb` index mode. For more information, refer to [](/solutions/security/detect-and-alert/using-logsdb-index-mode-with-elastic-security.md#logsdb-runtime-fields).
