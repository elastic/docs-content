...
You can also use a [data stream lifecycle](/manage-data/lifecycle/data-stream.md) to automate lifecycle management according to your retention requirements. {applies_to}`stack: ga 9.5, serverless: unavailable` You can optionally move older backing indices to {{search-snaps}} on the frozen tier while retention continues to enforce deletion timelines. Refer to [](/manage-data/lifecycle/data-stream/frozen-searchable-snapshots.md).
