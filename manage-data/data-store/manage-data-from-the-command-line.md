...
```bash
curl -u USER:PASSWORD <ELASTICSEARCH_URL>/my_index/_doc/_bulk -XPOST -H 'Content-Type: application/json' -d '
{"index": {}}
...
```bash
curl -u USER:PASSWORD <ELASTICSEARCH_URL>/my_index/_doc/06NqhW4BnhCSymaqFHQn -XPOST -H 'Content-Type: application/json' -d '{
...
```bash
curl -u USER:PASSWORD <ELASTICSEARCH_URL>/my_index/_doc/06NqhW4BnhCSymaqFHQn
```
...
```bash
curl -u USER:PASSWORD <ELASTICSEARCH_URL>/my_index/_doc/_search?q=title:T*
```
...
```bash
curl -u USER:PASSWORD <ELASTICSEARCH_URL>/my_index/_doc/_search?pretty=true -H 'Content-Type: application/json' -d '{
    "query": {
        "query_string": {"query": "*"}
    }
}'
...
```bash
curl -u USER:PASSWORD <ELASTICSEARCH_URL>/my_index/_doc/06NqhW4BnhCSymaqFHQn -XDELETE
```

To delete a whole index, here `my_index`:

```bash
curl -u USER:PASSWORD <ELASTICSEARCH_URL>/my_index -XDELETE
```
