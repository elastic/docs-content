...
{applies_to}`stack: ga` {applies_to}`serverless: unavailable` To change the file size limit, refer to [`fileUpload:maxFileSize`](kibana://reference/advanced-settings.md#kibana-general-settings) in **Advanced Settings**. The 60 MB limit for PDF and office files does not change.
