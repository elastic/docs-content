...
If you change the resources available on your {{ml}} nodes or make significant changes to the characteristics or cardinality of your data, the model memory requirements might also change. You can update the model memory limit for a job while it is closed. {applies_to}`stack: ga 9.5`{applies_to}`serverless: ga` In {{kib}}, open the closed job and select **Apply estimation** in the edit job flyout to set an estimated limit without running the estimate API manually. If you want to decrease the limit below the current model memory usage, however, you must clone and re-run the job.
