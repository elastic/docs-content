...
{applies_to}`stack: ga 9.5`{applies_to}`serverless: ga` When you edit an existing {{anomaly-job}} on the **Anomaly Detection Jobs** page, the edit job flyout includes a model memory estimation option. If your job is approaching its memory limits, select **Apply estimation** to update the `model_memory_limit` without recreating the job.
