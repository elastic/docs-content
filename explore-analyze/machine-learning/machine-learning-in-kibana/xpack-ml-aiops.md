...
{applies_to}`stack: ga 9.5` {applies_to}`serverless: ga` Each AIOps tool opens directly on its analysis page, where you can select or switch the {{data-source}} or saved Discover session from the picker in the page header, without going through a separate selection step.
