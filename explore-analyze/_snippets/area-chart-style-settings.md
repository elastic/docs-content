...
- {applies_to}`serverless: ga` {applies_to}`stack: ga 9.6+` **Fill**: To fade each series color, select **Gradient**. **Solid** is the default, so area charts saved before this setting existed stay solid until you change **Fill**. In the [Visualizations API](https://www.elastic.co/docs/api/doc/kibana/group/endpoint-visualizations), set `styling.areas.fill` to `solid` or `gradient`. If you omit `fill`, the value is `solid`.
