...
2. A flyout opens to let you configure the control. Specify:
    ...
    * {applies_to}`{"stack": "ga 9.4", "serverless": "ga"}` Whether the control allows a single selection or multiple selections. Multiple selections [require using the `MV_CONTAINS` function in your query](#esql-multi-values-controls).
