...
- **Color by value**: Apply colors to cell backgrounds, badges, or text based on values. Choose between:
  ...
  - {applies_to}`stack: ga 9.4` {applies_to}`serverless: ga` **Badge**: Display cell values as colored badges with palette-driven background colors. Empty, null, and `NaN` values are not displayed as badges and instead render as plain text.
