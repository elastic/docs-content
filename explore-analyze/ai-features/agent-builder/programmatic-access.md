...
- **[MCP server](mcp-server.md)**: A standardized interface that allows external MCP clients (such as Claude Desktop or Cursor) to access {{agent-builder}} tools using API keys or OAuth.
- **[Application connections](/deploy-manage/app-connections/oauth-clients.md)**: OAuth 2.1 client registration and connection management for interactive MCP access on {{serverless-short}} projects.
