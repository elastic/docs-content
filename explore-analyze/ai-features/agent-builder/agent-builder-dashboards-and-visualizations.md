...
| [Vega-Lite visualizations](/explore-analyze/visualize/custom-visualizations-with-vega.md) {applies_to}`stack: ga 9.5` {applies_to}`serverless: ga` | Used for chart shapes that Lens can't express, such as small multiples, layered or combination charts, scatter plots, and bubble charts, or when you ask for a Vega or custom visualization. The agent supports Vega-Lite specifications only, not raw Vega. Shapes that Vega-Lite can't express, such as Sankey, radar, or network diagrams, fall back to the closest supported chart. |
