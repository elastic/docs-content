...
:   The button in the {{kib}} top header that opens sidebar mode so you can chat with an agent from any page.
...
Attachment
:   Data or context added to a chat message, such as an alert flyout, a {{kib}} object, or a file. Attachments can trigger an agent to invoke the skill most relevant to the attachment type.
...
Chat history
:   The persisted record of previous conversations between a user and the agents they've used. Chat history is shared across standalone mode and sidebar mode.
...
Conversation
:   A single exchange or thread between a user and an agent in Agent Chat. Conversations preserve message history, agent identity, and any attachments used.
...
:   The Elastic AI Agent, which is automatically created in each {{kib}} space and acts as the starting agent for new conversations.
...
:   Elastic's managed service for running LLMs on Elastic infrastructure, used by Elastic Managed LLMs.
...
Flyout mode
:   Earlier name for sidebar mode. The two terms refer to the same chat panel; **sidebar mode** is the current name used in the documentation.
...
:   The {{kib}} settings page where you configure the default model and other generative-AI options that affect {{agent-builder}}.
...
MCP
:   The [Model Context Protocol](https://modelcontextprotocol.io/), an open standard for connecting AI assistants to external tools and data sources. {{agent-builder}} both _exposes_ tools through an MCP server and _consumes_ tools from remote MCP servers as MCP tools.

MCP connector {applies_to}`stack: preview 9.3+`
:   A {{kib}} connector that points {{agent-builder}} at a remote MCP server so its tools can be imported as MCP tools.
...
:   A reusable bundle of skills and supporting capabilities that can be assigned to an agent as a single unit. Plugins make it easier to share groups of related skills across agents.
