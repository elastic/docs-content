...
| **Include user data in traces** {applies_to}`{stack: ga 9.6+, serverless: ga}` | `agentBuilder:tracing:includeUserData` | Records the real user ID and username of the person who ran each conversation round, instead of a hash alone. |
...
- **The conversation round span** {applies_to}`{stack: ga 9.6+, serverless: ga}`: Stores the conversation title and the user identity. Only the root `invoke_agent` span carries these, one per conversation round. The nested agent, model, and tool spans do not. For the field names, the settings each one depends on, and how to select that span, refer to [Conversation round attributes](agent-traces-dashboard.md#conversation-round-attributes).
...
{applies_to}`{stack: ga 9.6+, serverless: ga}` When **Include user data in traces** is on, traces identify who ran each conversation round. Anyone who can read the data stream can see which user ran which round, so review this section before you turn that setting on.
