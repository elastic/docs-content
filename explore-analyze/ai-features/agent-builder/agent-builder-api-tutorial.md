...
<!-- [TODO-CHECK] Note the API field is camelCase `askUser` in an otherwise snake_case API, and its OAS description is the weak string "Optional tool call policy to control tool call confirmation behavior". Both are kibana-repo fixes, not this PR. Worth raising with Lisa Cawley if we document the field here. -->
