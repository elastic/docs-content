...
:::{warning}
Self-managed LLMs work well for {{elastic-sec}}'s [AI Assistant](/solutions/security/ai/ai-assistant.md). We do not recommend them for [Attack Discovery](/solutions/security/ai/attack-discovery/index.md). For Attack Discovery, choose a connector that uses a model from the [LLM performance matrix for {{elastic-sec}}](/solutions/security/ai/large-language-model-performance-matrix.md).
:::
