...
6. In the **Data Stream** panel, provide a **Data stream title** and **Data stream description**, then select a [**Data collection method**](beats://reference/filebeat/configuration-filebeat-options.md). The data collection method determines how the integration ingests the data (for example, from an S3 bucket, an HTTP endpoint, or a file stream).
