...
These examples demonstrate how to use Painless in {{transforms}}. You can learn more about the Painless scripting language in the [Painless guide](elasticsearch://docs/reference/scripting-languages/painless/painless.md).
...
Check the [scope of scripts](elasticsearch://docs/reference/data-analysis/aggregations/search-aggregations-metrics-scripted-metric-aggregation.md#scripted-metric-aggregation-scope) for detailed explanation on the respective scripts.
...
This example shows you how to get the duration of a session by client IP from a data log by using [bucket script](elasticsearch://docs/reference/data-analysis/aggregations/search-aggregations-pipeline-bucket-script-aggregation.md). The example uses the {{kib}} sample web logs dataset.
