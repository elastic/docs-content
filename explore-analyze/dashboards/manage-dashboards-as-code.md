...
- **Skip the data view**, because {{esql}} queries indices by name rather than through a saved data view.
...
- **Data views**: create them with a chosen ID, either through the [Data Views API]({{kib-apis}}operation/operation-createdataviewdefaultw) by passing an `id` in the POST request, or in the UI by setting a **Custom data view ID** under [advanced settings](../find-and-organize/data-views.md#settings-create-pattern). Use human-readable IDs, such as `logs-prod`, so they are easy to reuse and recognize.
