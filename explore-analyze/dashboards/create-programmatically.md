...
description: Compare the Dashboards API, Lens Visualizations API, and AI-powered tools to create and manage Kibana dashboards and visualizations programmatically.
applies_to:
  stack: ga
  serverless: ga
products:
...
In addition to the {{product.kibana}} UI, you can create and manage dashboards and visualizations using REST APIs or AI-powered tools. This is useful when you need to automate dashboard deployments, manage them in version control, build tooling around the dashboard lifecycle, or generate dashboards from natural language instructions without touching the UI.
...
| [Lens Visualizations API](#lens-visualizations-api) | Reusable visualization libraries shared across dashboards | Saved visualization object |
...
The Dashboards API provides full CRUD access to dashboards, including their panels, controls, sections, and display options. Panels are defined inline in the request body as JSON, which makes it straightforward to store dashboard definitions in version control and deploy them through automated pipelines.
...
- Create dashboards that include [ES|QL](/explore-analyze/query-your-data-with-esql.md)-powered Lens visualizations inline — this is currently the only programmatic path for ES|QL charts

The API supports all panel types that have a defined schema, including Lens visualizations, Discover sessions, markdown panels, and filter controls. Panels that lack a schema — such as Maps and Links — return an error on write.
...
The Lens Visualizations API lets you create and manage Lens visualizations as standalone saved objects in the Kibana Visualizations library. A saved visualization can then be embedded in one or more dashboards by referencing its ID, rather than duplicating the visualization definition in each dashboard.

Use the Lens Visualizations API when you need to:
...
{{agent-builder}} agents can create and update dashboards through natural language chat in the {{product.kibana}} UI. Describe what you want to visualize and the agent generates a dashboard with [{{esql}}](/explore-analyze/query-your-data-with-esql.md)-powered [Lens](/explore-analyze/visualize/lens.md) panels. Dashboards are in-memory by default — attached to the conversation and not saved until you choose to save them — so you can iterate freely before committing.
...
{{agent-builder}} creates ES|QL-powered Lens visualizations, markdown panels, and collapsible sections. If you need DSL-based visualizations or panel types not supported by the agent, use the Dashboards API directly.
...
The [kibana-dashboards agent skill](https://github.com/elastic/agent-skills/tree/main/skills/kibana/kibana-dashboards) is an open-source skill for developers building AI agents and LLM-powered applications. It gives language models the context and instructions they need to generate valid dashboard definitions and interact with the Dashboards API on behalf of a user.
...
- Extending or customizing how an LLM interacts with the Dashboards API beyond what {{agent-builder}} provides out of the box
