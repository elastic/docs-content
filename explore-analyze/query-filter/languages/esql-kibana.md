...
The {{esql}} editor includes a search bar that helps you write a query without writing the full {{esql}} syntax. To open it, select the {icon}`magnify` search icon in the editor's toolbar, or press {kbd}`cmd+k` (Mac) or {kbd}`ctrl+k` (Windows/Linux). You can then draft a query using:
