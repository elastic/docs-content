...
Each row shows two columns for the example query: a column with the `@timestamp` field and a column with the full document. To display specific fields from the documents, use the [`KEEP`](elasticsearch://reference/query-languages/esql/_snippets/commands/layout/keep.md) command:
...
You can also use the `?_tstart` and `?_tend` parameters with the [`BUCKET`](elasticsearch://reference/query-languages/esql/_snippets/functions/layout/bucket.md) function to create auto-incrementing time buckets in {{esql}} [visualizations](#esql-kibana-visualizations). For example:
...
You can also limit the time range using the [`WHERE`](elasticsearch://reference/query-languages/esql/_snippets/commands/layout/where.md) command and the [`NOW`](elasticsearch://reference/query-languages/esql/_snippets/functions/layout/now.md) function. For example, if the timestamp field is called `timestamp`, to query the last 15 minutes of data:
...
* The user interface to filter data is not enabled when Discover is in {{esql}} mode. To filter data, write a query that uses the [`WHERE`](elasticsearch://reference/query-languages/esql/_snippets/commands/layout/where.md) command instead.
