...
2. Select the data source to take into account.
3. Type the text you want to search for.
4. Submit your search by pressing **Enter**. A new {{esql}} query is generated and overwrites the current query. It is made of a `FROM` command based on the data sources you selected, and of a `WHERE KQL()` command that contains the text you typed in the search bar. Any query that you have previously run is saved in the query history if you need to restore it. 
