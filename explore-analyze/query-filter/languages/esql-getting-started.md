...
The [`FROM`](elasticsearch://reference/query-languages/esql/_snippets/commands/layout/from.md) source command returns a table with documents from a data stream, index, or alias. Each row in the resulting table represents a document. This query returns up to 1000 documents from the `sample_data` index:
...
For example, you can use the [`LIMIT`](elasticsearch://reference/query-languages/esql/_snippets/commands/layout/limit.md) command to limit the number of rows that are returned, up to a maximum of 10,000 rows:
...
Another processing command is the [`SORT`](elasticsearch://reference/query-languages/esql/_snippets/commands/layout/sort.md) command. By default, the rows returned by `FROM` don’t have a defined sort order. Use the `SORT` command to sort rows on one or more columns:
...
Use the [`WHERE`](elasticsearch://reference/query-languages/esql/_snippets/commands/layout/where.md) command to query the data. For example, to find all events with a duration longer than 5ms:
...
`WHERE` supports several [operators](elasticsearch://reference/query-languages/esql/functions-operators/operators.md). For example, you can use [`LIKE`](elasticsearch://reference/query-languages/esql/_snippets/operators/layout/like.md) to run a wildcard query against the `message` column:
...
There are many other processing commands, like [`KEEP`](elasticsearch://reference/query-languages/esql/_snippets/commands/layout/keep.md) and [`DROP`](elasticsearch://reference/query-languages/esql/_snippets/commands/layout/drop.md) to keep or drop columns, [`ENRICH`](elasticsearch://reference/query-languages/esql/_snippets/commands/layout/enrich.md) to enrich a table with data from indices in {{es}}, and [`DISSECT`](elasticsearch://reference/query-languages/esql/_snippets/commands/layout/dissect.md) and [`GROK`](elasticsearch://reference/query-languages/esql/_snippets/commands/layout/grok.md) to process data. Refer to [Processing commands](elasticsearch://reference/query-languages/esql/commands/processing-commands.md) for an overview of all processing commands.
...
{{esql}} can not only be used to query your data, you can also use it to aggregate your data. Use the [`STATS`](elasticsearch://reference/query-languages/esql/_snippets/commands/layout/stats-by.md) command to calculate statistics. For example, the median duration:
...
To track statistics over time, {{esql}} enables you to create histograms using the [`BUCKET`](elasticsearch://reference/query-languages/esql/_snippets/functions/layout/bucket.md) function. `BUCKET` creates human-friendly bucket sizes and returns a value for each row that corresponds to the resulting bucket the row falls into.

Combine `BUCKET` with [`STATS`](elasticsearch://reference/query-languages/esql/_snippets/commands/layout/stats-by.md) to create a histogram. For example, to count the number of events per hour:
...
{{esql}} enables you to [enrich](elasticsearch://reference/query-languages/esql/esql-enrich-data.md) a table with data from indices in {{es}}, using the [`ENRICH`](elasticsearch://reference/query-languages/esql/_snippets/commands/layout/enrich.md) command.
