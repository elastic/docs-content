...
**Description**: A full-text search option, in the form of a predicate, available in Elasticsearch SQL that gives the user control over powerful [match](elasticsearch://docs/reference/query-languages/query-dsl-match-query.md) and [multi_match](elasticsearch://docs/reference/query-languages/query-dsl-multi-match-query.md) {{es}} queries.
...
**Description**: Just like `MATCH`, `QUERY` is a full-text search predicate that gives the user control over the [query_string](elasticsearch://docs/reference/query-languages/query-dsl-query-string-query.md) query in {{es}}.
