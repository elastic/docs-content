...
| {applies_to}`stack: preview 9.4` {applies_to}`serverless: preview` **PromQL** | [Prometheus Query Language (PromQL)](https://prometheus.io/docs/prometheus/latest/querying/basics/) syntax for querying time series data, supported in {{es}} as a source command within {{esql}}.<br><br>[Full reference](elasticsearch://reference/query-languages/esql/commands/promql.md) | Query time series data using familiar Prometheus syntax in the {{esql}} editor.<br><br>[PromQL in Kibana](languages/esql-kibana.md#esql-kibana-query-bar) | [`_query`](https://www.elastic.co/docs/api/doc/elasticsearch/group/endpoint-esql) |
