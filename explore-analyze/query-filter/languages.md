...
| **{{esql}}** | Introduced in **8.11**, the Elasticsearch Query Language ({{esql}}) is a piped query language for filtering, transforming, and analyzing data.<br><br>[Full language reference](elasticsearch://reference/query-languages/esql.md) | You can use {{esql}} in multiple {{kib}} applications for querying, visualizing, and analyzing data.<br><br>{applies_to}`stack: preview 9.4` {applies_to}`serverless: preview` {{esql}} also supports [Prometheus Query Language (PromQL)](elasticsearch://reference/query-languages/esql/commands/promql.md) syntax for time series queries through the `PROMQL` source command.<br><br>[{{esql}} in Kibana](languages/esql-kibana.md) | [`_query`](https://www.elastic.co/docs/api/doc/elasticsearch/group/endpoint-esql) |
