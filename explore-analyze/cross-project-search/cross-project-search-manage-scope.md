...
{{observability}} apps have limited {{cps-init}} support. APM and Infrastructure use the header [scope selector](#cps-in-kibana). SLOs store project scope on each SLO instead of using the header selector. Other {{observability}} apps remain scoped to the origin project. The following table shows how each {{observability}} app behaves with {{cps-init}}:
