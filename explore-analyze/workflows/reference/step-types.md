...
| [`security.createExceptionListItem`](/explore-analyze/workflows/steps/exceptions.md#security-createexceptionlistitem) {applies_to}`stack: ga 9.6+` {applies_to}`serverless: ga` | Security | Add an exception item to an existing exception list, with optional idempotency via `item_id`. |
| [`security.createRuleException`](/explore-analyze/workflows/steps/exceptions.md#security-createruleexception) {applies_to}`stack: ga 9.6+` {applies_to}`serverless: ga` | Security | Add an exception item to a rule's own default exception list, with optional idempotency via `item_id`. |
