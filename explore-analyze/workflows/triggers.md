...
* Auditing rule management actions, syncing rule inventory with external systems, or notifying a channel when rules change in the {{alerting-v2-system}} {applies_to}`stack: experimental 9.5+` {applies_to}`serverless: ga`
...
* **Event-driven**:
  ...
  * Alert episode lifecycle triggers provide `event.episodeId`, `event.ruleId`, and `event.spaceId`. {applies_to}`stack: experimental 9.5+` {applies_to}`serverless: ga`
  * {{alerting-v2-system-cap}} rule lifecycle triggers provide `event.rule.ruleId` and `event.rule.spaceId`. {applies_to}`stack: experimental 9.5+` {applies_to}`serverless: ga`
