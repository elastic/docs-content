...
* **Security triggers** fire when {{elastic-sec}} detection alerts, attack discoveries, or document notes change {applies_to}`stack: preview 9.6+` {applies_to}`serverless: preview`. The family includes `security.alertStatusChanged`, `security.alertTagsChanged`, `security.alertAssigneesChanged`, `security.attackStatusChanged`, `security.attackTagsChanged`, `security.attackAssigneesChanged`, `security.noteCreated`, and `security.noteUpdated`.
...
* **Event-driven**:
  ...
  * Security triggers provide `event.alertIds` or `event.attackIds` alongside `event.truncated`, and event-specific fields (status changes carry `status` and `previousStatuses`; tag and assignee changes carry the added and removed values). The note triggers provide `event.noteId`, `event.documentId`, and the acting user. {applies_to}`stack: preview 9.6+` {applies_to}`serverless: preview`
