...
The sidebar runs an {{agent-builder}} conversation, so the exchange is saved to your [conversation history](/explore-analyze/ai-features/agent-builder/chat.md#find-conversation-history) alongside conversations you start elsewhere.
...
- Run a [test run](/explore-analyze/workflows/authoring-techniques/use-yaml-editor.md#workflows-test-vs-production-runs) from the workflow editor to verify the workflow before you enable production triggers.
