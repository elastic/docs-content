...
| `if` | top level | string | No | Boolean or KQL expression that determines whether the step runs. When it's false, the workflow continues without requesting approval. Refer to [Skip a single step](/explore-analyze/workflows/steps/if.md#workflows-step-level-if). |
