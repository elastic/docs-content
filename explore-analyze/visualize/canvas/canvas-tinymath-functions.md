...
{applies_to}`stack: ga 9.5` {applies_to}`serverless: ga` A TinyMath expression can contain up to 20 levels of parenthesis nesting and up to 1,000 characters. Expressions that exceed either limit are rejected before they're parsed. Typical expressions stay well within both limits.
