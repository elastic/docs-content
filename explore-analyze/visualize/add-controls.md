...
{applies_to}`stack: preview 9.5` {applies_to}`serverless: preview` You can also ask [{{agent-builder}}](/explore-analyze/ai-features/agent-builder/agent-builder-dashboards-and-visualizations.md) to add Options list and Range slider controls when it creates or updates a dashboard through chat.
