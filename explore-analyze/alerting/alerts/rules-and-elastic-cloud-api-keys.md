...
{{kib}} creates an API key when a rule is created or edited, then uses it to authorize execution on every run. {{es}} API keys are scoped to a user's **privileges**, whereas {{ecloud}} API keys are scoped to a user's **roles**.
