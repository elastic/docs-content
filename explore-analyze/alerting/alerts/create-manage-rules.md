...
When you create or edit a rule, the [project picker](/explore-analyze/cross-project-search.md#cps-in-kibana) in the {{kib}} header shows the current {{cps}} scope but you cannot change it from the rule page. To change which projects a rule queries, update the {{cps}} scope configured for the space.
