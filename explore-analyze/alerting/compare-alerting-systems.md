...
| **Alert and signal data** | In-place updates, limited query support | Append-only [rule events](experimental-alerting-system/alerts/rule-event-data-model.md) for alert episodes and signals, [queryable with {{esql}} in Discover](experimental-alerting-system/alerts/query-alerts-and-signals-in-discover.md) | Watch history index |
