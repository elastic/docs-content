...
| **Rule event data** | In-place updates, limited query support | Append-only [rule events](experimental-alerting-system/alerts/rule-event-data-model.md), [queryable with {{esql}} in Discover](experimental-alerting-system/alerts/query-alerts-and-signals-in-discover.md) | Watch history index |
