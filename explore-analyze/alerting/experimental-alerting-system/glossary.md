...
**Signal**
:   A rule event recorded when a rule runs in Signal mode. Signals are stored in `.rule-events` and queryable in Discover, but they don't open an alert episode or trigger notifications. To learn more, refer to [Observe and analyze signals](observe-and-analyze-signals.md) and [Rule mode](rules/configure-rule-mode.md).
