...
|---|---|
| Rule executes | The rule uses the [API key](../alerts/alerting-setup.md#alerting-authorization) of the user who last saved it. That key determines what data the rule can query.<br>{applies_to}`stack: experimental 9.6+` {applies_to}`serverless: experimental` To replace that key with one based on your current credentials, refer to [Rotate a rule's API key](rules/view-manage-rules.md#rotate-rule-api-key). |
...
| Stale or not valid API key | The stored key is no longer valid, for example because an administrator deleted or expired a role it depended on. | An API key error in rule execution history. | Refresh the key by saving the rule again, or by turning the rule off and back on.<br>{applies_to}`stack: experimental 9.6+` {applies_to}`serverless: experimental` You can also select **Update API key** from the rule's actions menu. Refer to [Rotate a rule's API key](rules/view-manage-rules.md#rotate-rule-api-key). |
