...
This page walks through what happens at each step after a rule runs on its schedule, broken down by mode. Both modes begin the same way: {{kib}} writes a [rule event](rules/rule-event-field-reference.md) for each matching row. Use this page to understand how the different components of the {{alerting-v2-system}} interact.
