...
3. {applies_to}`stack: ga 9.6+` {applies_to}`serverless: ga` An action policy scoped to the `checkout` rule tag invokes an on-call workflow that sends a Slack message.

   {applies_to}`stack: removed 9.6+, experimental =9.5` {applies_to}`serverless: unavailable` An action policy with a `rule.tags: "checkout"` matcher invokes an on-call workflow that sends a Slack message.
