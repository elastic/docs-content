...
3. {applies_to}`serverless: ga` {applies_to}`stack: ga 9.6+` An action policy scoped to the `checkout` rule tag invokes an on-call workflow that sends a Slack message.
