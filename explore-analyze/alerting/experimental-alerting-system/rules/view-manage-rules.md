...
- **Overview**: For rules that group matches into an alert episode, shows a color-coded alert activity timeline per series, with summary statistics (alert episodes started, recovered, still open, and median duration) and a link to view matching alert episodes on the **Alerts** page. The **Artifacts** section lists the dashboards linked to the rule, which you can open in a new tab. You can also change which dashboards are linked from this section. Refer to [Tags, runbooks, and dashboards](configure-rule-artifacts.md).
