...
- **Signals can also store severity** - Events with `type: signal` also include `severity` when the query emits a recognized value. Signals don't open episodes, so action policy matchers that filter by severity don't apply.
