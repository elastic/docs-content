...
- **Signal documents also store severity** - Signal mode events also include `severity` when the query emits a recognized value. Signals don't open episodes, so action policy matchers that filter by severity don't apply.
