...
This page explains what a rule event is and how Signal mode and Alert mode use those events. For how the shared schema works conceptually, refer to [Alert data model](../alerts/alert-data-model.md). For the complete field list, refer to [Field reference](../alerts/field-reference.md#rule-events-field-schema).
...
- **Signal mode**: Each match is a `type: signal` rule event. That event is the whole output. It has no `episode.*` fields, doesn't open an alert episode, and doesn't trigger notifications. Query these events in Discover, build dashboards from them, or feed them into a follow-on Alert mode rule.
