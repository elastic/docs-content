...
When a rule finds a match, {{kib}} writes a **rule event** to `.rule-events`. Each matching row in a run becomes its own event, and {{kib}} never overwrites those events. The rule's configuration determines whether that event is grouped into an alert episode. Events that aren't part of an alert episode have `type: signal`.

Use this page to query `.rule-events` with confidence: replay an alert episode's history, investigate events with `type: signal`, or build dashboards from rule output. For the stored schema, refer to [Rule event data model](../alerts/rule-event-data-model.md). For the complete field list, refer to [Field reference](../alerts/field-reference.md#rule-events-field-schema). For how events with `type: alert` relate to an [alert episode](../alerts.md), refer to [Alerts](../alerts.md).
...
- [View and manage alerts](../alerts/view-and-manage-alerts.md): Where alert episodes appear in the UI, with triage actions and alert episode details.
