...
An action policy is a saved object in your space that controls notification routing. It is not attached to a rule. It's global within the space, so when an episode is produced, the system evaluates all enabled, unsnoozed policies in the space and decides which ones apply.
...
| Destinations | One or more workflows to invoke when all the above pass. |
...
Rules don't own policies. A rule can't say "notify team X when I fire." Instead, team X creates an action policy that uses a KQL matcher to pick up the right episodes.
