---
navigation_title: Throttling
applies_to:
  serverless: preview
products:
  - id: kibana
description: "Limit {{alerting-v2}} notification frequency by setting a minimum interval between notifications for the same group."
---

# {{alerting-v2}} throttle [throttle-v2]

**Throttling** sets a minimum interval between notifications for the same **notification group**. It reduces notification volume without affecting rule execution or **alert episode** lifecycle state.
