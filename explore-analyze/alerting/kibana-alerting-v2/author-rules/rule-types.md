...
You can create multi-level threshold rules with different severity levels:

```esql
FROM metrics-*
| STATS avg_cpu = AVG(system.cpu.total.pct) BY host.name
| EVAL severity = CASE(
    avg_cpu > 0.95, "critical",
    avg_cpu > 0.9, "high",
    avg_cpu > 0.8, "warning",
    "ok"
  )
| WHERE severity != "ok"
| KEEP host.name, avg_cpu, severity
```

## Change detection

Alert when a value deviates from a previous period. Useful for detecting spikes or regressions.
...
Alert when a previously reporting source stops sending data. Requires a longer lookback window to establish that the source was recently active.
...
**Lookback window**: Set to a period longer than the expected gap (for example, `12h`) so the query can find the host's last data point.

You can combine no-data detection with metric thresholds in a single rule:
...
Alert on counter-based rates like pod restarts.

```esql
FROM metrics-*
| STATS
    current = MAX(kubernetes.pod.restart.count) WHERE @timestamp >= NOW() - 5 MINUTES,
    previous = MAX(kubernetes.pod.restart.count) WHERE @timestamp < NOW() - 5 MINUTES
  BY kubernetes.pod.name
| EVAL restart_rate = current - previous
| WHERE restart_rate > 0
| KEEP kubernetes.pod.name, restart_rate
```
...
Alert when an SLO error budget is burning too fast, using multi-window burn rate calculations.
