...
<!-- TODO: Issue #6837 confirms that rule.kind is a filterable field in the action policy filter dropdown. Add a row for it here once the full field contract is confirmed (accepted values, KQL syntax). The UI label for the detect-mode kind is "Signal" — do NOT use "Detection only", which was the previous label and has been renamed. -->
