...
**Result:** {{kib}} opens a new, unsaved dashboard with your response code chart already on it. The chart is saved to the **Visualize Library** and linked to the dashboard, so any edits you make to it later will be reflected on every dashboard that uses it. Panels you add directly to a dashboard without saving to the library are local to that dashboard only.
