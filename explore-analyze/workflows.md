...
Workflows are defined in YAML. In the YAML editor, describe _what_ the workflow should do, and the platform handles execution. The example below uses the stack 9.5+ and serverless structure, where `inputs` is defined under the `manual` trigger. On stack 9.4 and earlier, `inputs` sits at the workflow root. Refer to [Workflow anatomy](/explore-analyze/workflows/authoring-techniques/anatomy.md#workflows-anatomy-inputs) for both forms.
