...
* To show or hide sections of the Discover view:
  * {applies_to}`serverless:` {applies_to}`stack: ga 9.4` Use the **Panels visibility** button group to show or hide the chart, results table, and fields list independently. The button group stays in a fixed position regardless of which sections are shown or hidden.
