...
:::{note}
:applies_to: {"stack": "preview 9.5", "serverless": "preview"}
On large datasets, you can return faster, estimated results for a query that uses a single `STATS` command. Select the {icon}`bolt` Fast mode button next to the query bar. Refer to [Get faster results with Fast mode](/explore-analyze/query-filter/languages/esql-kibana.md#approximation-fast-mode).
:::
