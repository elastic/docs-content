...
- {applies_to}`serverless: preview` {applies_to}`stack: preview 9.6+` To export the session configuration as JSON, select {icon}`ellipsis` **More** → {icon}`download` **Export** → **Export JSON**. Refer to [Export a Discover session as JSON](save-open-search.md#export-discover-session-json).
