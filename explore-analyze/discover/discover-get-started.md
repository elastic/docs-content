...
4. {applies_to}`stack: ga 9.5` {applies_to}`serverless: ga` Optionally, use **Add to dashboard** to also add the session as a panel on a dashboard. For details, refer to [Save a Discover session](save-open-search.md#_save_a_discover_session).
5. Select **Save**.
