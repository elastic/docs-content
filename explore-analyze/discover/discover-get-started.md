...
4. {applies_to}`stack: ga 9.5` {applies_to}`serverless: ga` Optionally, in **Add to dashboard**, add the session to a dashboard at the same time as you save it. For details, refer to [Save a Discover session](save-open-search.md#_save_a_discover_session).
5. Select **Save**.
