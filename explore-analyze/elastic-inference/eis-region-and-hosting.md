...
All {{infer}} requests sent through EIS are routed to the nearest region, regardless of where your {{es}} deployment or {{serverless-short}} project is hosted. For deployments and projects in an EU region, we will route to the nearest EIS installation in an EU region.
