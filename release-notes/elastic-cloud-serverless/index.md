...
* Adds troubleshooting view for requests between nodes in the Service Map [#248646]({{kib-pull}}248646)
* Makes APM SLOs viewable directly from the Service Inventory page [#249374]({{kib-pull}}249374)
* Adds Cloud Forwarder onboarding tile to the Observability Solution onboarding page [#250325]({{kib-pull}}250325)
...
* Shows RED (Rate, Errors, Duration) metrics charts for traces queries in APM [#249909]({{kib-pull}}249909)
* Introduces a severity color palette for color mapping in visualizations [#250198]({{kib-pull}}250198)
* Adds auto-push option to case connectors, enabling automatic case synchronization [#249251]({{kib-pull}}249251)
...
* Exposes `configuration_overrides` in the agent builder and converse API [#249256]({{kib-pull}}249256)
...
* Adds IP and Boolean type support for the {{esql}} `FIRST` and `LAST` functions [#140097]({{es-pull}}140097)
...
* Adds `max_batch_size` setting to Elastic Inference Service sparse embedding settings [#141185]({{es-pull}}141185)
...
* Adds native operations for scoring floats, improving search performance [#140169]({{es-pull}}140169)
...
* Limits IDs query to the `max_result_window` setting (default: 10,000) for improved resource protection [#140515]({{es-pull}}140515)
* Introduces adaptive block hash for improved {{esql}} aggregation performance with high-cardinality data [#141237]({{es-pull}}141237)
* Adds configurable quantization levels (1, 2, or 4 bits) for `bbq_disk` dense vector index type [#139944]({{es-pull}}139944)
* Enables scroll API cross-project search compatibility [#140977]({{es-pull}}140977)
...
* Supports point-in-time (PIT) context relocation when nodes gracefully shut down and shards relocate [#137675]({{es-pull}}137675)
* Releases {{esql}} `FIRST` and `LAST` aggregation functions as tech preview [#141490]({{es-pull}}141490)
* Adds documentation for the new `t-digest` field type [#140478]({{es-pull}}140478)
...
* Fixes the disabled connector button not being clickable [#250921]({{kib-pull}}250921)
* Fixes alert info in the privileged user monitoring table to re-query when time range changes [#250618]({{kib-pull}}250618)
* Fixes occasional file preview corruption during file upload [#250532]({{kib-pull}}250532)
* Fixes time range being lost when switching to the retention tab [#247544]({{kib-pull}}247544)
...
* Fixes waterfall summary width in traces within Discover [#250556]({{kib-pull}}250556)
* Propagates {{esql}} execution errors correctly to users [#250605]({{kib-pull}}250605)
* Handles empty results gracefully in {{esql}} alerting rule execution [#250759]({{kib-pull}}250759)
* Adjusts panel height in Discover for improved layout [#250778]({{kib-pull}}250778)
* Fixes metrics grid titles not updating when column order changes [#250963]({{kib-pull}}250963)
...
* Fixes {{esql}} to use standard logic for conflicting metric types in time-series data, preserving original type information [#141496]({{es-pull}}141496)
* Appends an implicit limit to {{esql}} subqueries with unbounded sort for consistent behavior [#141025]({{es-pull}}141025)
* Fixes {{ml}} model cache to evict old models before loading new ones, preventing circuit breaker exceptions [#140844]({{es-pull}}140844)
