...
Applies to: ECH 9.2.0 deployments hosted on AWS, {{serverless-short}} deployments hosted on AWS

**Impact**

If your ECH or {{serverless-short}} deployment is hosted on AWS, new Cloud Security Posture Management (CSPM) and Asset Inventory integrations will fail to produce findings when deployed using agent-based deployment. ECH and {{serverless-short}} deployments hosted on GCP or Azure are not affected. Integrations that use agentless deployment are not affected. 
