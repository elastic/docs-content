...
% TODO: Confirm with the PR author whether this belongs in the 9.5 release notes. This is gated behind the `entityAttachmentsEnabled` experimental feature flag, which defaults to `false` (OFF) in 9.5, so it may not be generally to customers.
...
* Adds a {{elastic-defend}} advanced policy option for Windows that lets you disable enrichment of DLL Search Order Hijacking detection events [#272068]({{kib-pull}}272068).
...
* Raises the default file size limit for the `upload` response action to 500 MB and effectively removes the maximum, configurable via `xpack.securitySolution.maxUploadResponseActionFileBytes` [#275850]({{kib-pull}}275850).
