...
| **Disk Usage - Used (%)** | Percentage of disk space used. <br><br>{applies_to}`ga: 9.0-9.2`**Field Calculation**: `1 - sum(metrics.system.filesystem.usage, kql='state: free') / sum(metrics.system.filesystem.usage)`<br><br>{applies_to}`ga: 9.3`**Field Calculation**: `max(metrics.system.filesystem.utilization)`<br> |
