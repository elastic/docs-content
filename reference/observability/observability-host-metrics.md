...
| {applies_to}`stack: ga 9.0` **Disk Usage - Used (%)** | Percentage of disk space used. <br><br>**Field Calculation**: `1 - sum(metrics.system.filesystem.usage, kql='state: free') / sum(metrics.system.filesystem.usage)`<br> |
| {applies_to}`stack: ga 9.3` **Disk Usage - Max (%)** | Percentage of disk space used. <br><br>**Field Calculation**: `max(metrics.system.filesystem.utilization)`<br> |
