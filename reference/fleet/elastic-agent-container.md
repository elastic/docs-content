...
* When {{agent}} runs inside a container, it cannot be upgraded through {{fleet}} as it expects that the container itself is upgraded. For a full summary of upgrade support across all install methods, refer to [Restrictions](/reference/fleet/upgrade-elastic-agent.md#upgrade-agent-restrictions).
