...
6. In Step 2 on the page, you have two options:
    ...
    2. If you already have an {{agent}} policy created, on the **Existing hosts** tab use the drop-down menu to specify one or more agent policies that you’d like to add the integration to. Note that this feature, known as **reusable integration policies**, is available only for certain subscription levels. For more information, refer to [{{stack}} subscriptions](https://www.elastic.co/subscriptions).
...
If your [{{stack}} subscription level](https://www.elastic.co/subscriptions) supports **per policy output assignment**, you can change the output of a policy to send data to a different output.
...
3. Set **Output for integrations** and (optionally) **Output for agent monitoring** to use a different output, for example, {{ls}}. You might need to scroll down to see these options.
    ...
    Unable to select a different output? Confirm that your [{{stack}} subscription level](https://www.elastic.co/subscriptions) supports **per policy output assignment** in {{fleet}}.
