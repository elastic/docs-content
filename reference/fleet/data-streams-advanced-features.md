...
These features can be enabled and disabled for {{fleet}}-managed data streams by using the index template API and a few key settings. Note that in versions 8.17.0 and later, synthetic `_source` is available only for certain [{{stack}} subscription levels](https://www.elastic.co/subscriptions).
