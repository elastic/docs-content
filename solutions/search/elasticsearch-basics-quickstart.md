...
Create an index named `my-explicit-mappings-books` with explicit mappings. Pass each field’s properties as a JSON object. This object should contain the [field data type](elasticsearch://docs/reference/elasticsearch/mapping-reference/field-data-types.md) and any additional [mapping parameters](elasticsearch://docs/reference/elasticsearch/mapping-reference/mapping-parameters.md).
...
3. The `_shards` field contains information about the number of [shards](docs-content://docs/reference/glossary/index.md) that the search was executed on and the number that succeeded
...
You can use the [`match` query](elasticsearch://docs/reference/query-languages/query-dsl-match-query.md) to search for documents that contain a specific value in a specific field. This is the standard query for full-text searches.
