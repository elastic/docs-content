...
The ES|QL approach uses the [match (`:`) operator](elasticsearch://reference/query-languages/esql/_snippets/operators/layout/match_operator.md), which automatically detects the `semantic_text` field and performs the search on it. The query uses `METADATA _score` to sort by `_score` in descending order.
...
2. The [match (`:`) operator](elasticsearch://reference/query-languages/esql/_snippets/operators/layout/match_operator.md) is used on the `content` field for standard keyword matching
