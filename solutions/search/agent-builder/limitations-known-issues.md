...
When using {{agent-builder}}, total token usage will typically exceed the visible conversation text. Because {{agent-builder}} utilizes an agentic framework, a single user request often triggers multiple model calls (rounds) to process reasoning steps, execute tools, and interpret results.
...
:::{note}
As the conversation history grows and the agent performs more complex reasoning loops, the input and output token counts will increase multiplicatively for each round of execution.
:::
