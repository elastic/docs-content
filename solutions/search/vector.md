...
:   A system designed to store vector embeddings at scale and retrieve the most similar vectors to a query embedding, typically using approximate or exact k-nearest neighbor (kNN) search. {{es}} acts as a vector database when you store embeddings in [`dense_vector`](vector/dense-vector.md) or [`sparse_vector`](vector/sparse-vector.md) fields and query them for similarity. {{es}} can combine vector search with full-text search, structured filters, aggregations, and hybrid retrieval in one engine, so you can keep lexical, semantic, and operational queries on the same data and infrastructure.
...
- [Semantic search with ELSER](semantic-search/semantic-search-elser.md): Use built-in semantic search with explainable results, without external models.
- [Using Cohere with Elasticsearch](semantic-search/semantic-search-cohere.md): Generate embeddings using Cohere models via the Inference API and combine vector, hybrid search, reranking, and RAG in a single workflow.
