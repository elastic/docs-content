...
In our result set, we have many documents with the same `year` value. We can clean this up using the `collapse` parameter with our retriever. This, as with the standard [collapse](elasticsearch://docs/reference/elasticsearch/rest-apis/collapse-search-results.md) feature,
...
Highlighting is now also available for nested sub-retrievers matches. For example, consider the same `rrf` retriever as above, with a `knn` and `standard` retriever as its sub-retrievers. We can specify a `highlight` section, as defined in the [highlighting](elasticsearch://docs/reference/elasticsearch/rest-apis/highlighting.md) documentation, and compute highlights for the top results.
...
Now we can run an `rrf` retriever query and also compute [inner hits](elasticsearch://docs/reference/elasticsearch/rest-apis/retrieve-inner-hits.md) for the `nested_field.nested_vector` field, based on the `knn` query specified.
