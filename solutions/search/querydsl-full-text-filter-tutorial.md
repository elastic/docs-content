...
The [`match`](elasticsearch://docs/reference/query-languages/query-dsl-match-query.md) query is the standard query for full-text, or "lexical", search. The query text will be analyzed according to the analyzer configuration specified on each field (or at query time).
...
Use the [`minimum_should_match`](elasticsearch://docs/reference/query-languages/query-dsl-minimum-should-match.md) parameter to specify the minimum number of terms a document should have to be included in the search results.
...
When users enter a search query, they often don’t know (or care) whether their search terms appear in a specific field. A [`multi_match`](elasticsearch://docs/reference/query-languages/query-dsl-multi-match-query.md) query allows searching across multiple fields simultaneously.
...
Learn more about fields and per-field boosting in the [`multi_match` query](elasticsearch://docs/reference/query-languages/query-dsl-multi-match-query.md) reference.
...
This [`bool`](elasticsearch://docs/reference/query-languages/query-dsl-bool-query.md) query will return only blog posts in the "Breakfast" category.
...
1. Note the use of `category.keyword` here. This refers to the [`keyword`](elasticsearch://docs/reference/elasticsearch/mapping-reference/keyword.md) multi-field of the `category` field, ensuring an exact, case-sensitive match.
...
Often users want to find content published within a specific time frame. A [`range`](elasticsearch://docs/reference/query-languages/query-dsl-range-query.md) query finds documents that fall within numeric or date ranges.
...
::::{tip} 
Avoid using the `term` query for [`text` fields](elasticsearch://docs/reference/elasticsearch/mapping-reference/text.md) because they are transformed by the analysis process.
...
A [`bool`](elasticsearch://docs/reference/query-languages/query-dsl-bool-query.md) query allows you to combine multiple query clauses to create sophisticated searches. In this tutorial scenario it’s useful for when users have complex requirements for finding recipes.
...
1. The title contains "Spicy" and "Curry", matching our should condition. With the default [best_fields](elasticsearch://docs/reference/query-languages/query-dsl-multi-match-query.md#type-best-fields) behavior, this field contributes most to the relevance score.
