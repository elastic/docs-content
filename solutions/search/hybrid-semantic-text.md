...
Download the file and upload it to your cluster using the [Data Visualizer](../../manage-data/ingest/upload-data-files.md) in the {{ml-app}} UI. Name the index name as `test-data`. After your data is analyzed, name the index `test-data` in the  click **File settings**. Under **Field names**, assign `id` to the first column and `content` to the second. Click **Apply**, then **Import**. After the upload is complete, you will see an index named `test-data` with 182,469 documents.
