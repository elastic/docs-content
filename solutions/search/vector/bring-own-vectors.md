...
:::{tip}
The `dense_vector` type automatically uses quantization to reduce the memory footprint when searching float vectors. The type of quantization used by default depends on your product version. For details and to learn how to balance performance and accuracy, refer to [Dense vector quantization](elasticsearch://reference/elasticsearch/mapping-reference/dense-vector.md#dense-vector-quantization).
:::
