...
:::{note}
The `text_embedding` builder is one of three available [query vector builders](query-vector-builders.md). To generate a query vector from multimodal inputs like text and images, use the [`embedding` builder](query-vector-builders.md#embedding-query-vector-builder).
:::
...
For full `lookup` parameter details, refer to [Query vector builders — `lookup`](query-vector-builders.md#lookup-query-vector-builder).
