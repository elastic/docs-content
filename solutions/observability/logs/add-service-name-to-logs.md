...
For log data without a service name, use the [`add_fields` processor](docs-content://docs/reference/ingestion-tools/fleet/add_fields-processor.md) to add the `service.name` field. You can add the processor in an integration’s settings or in the {{agent}} or {{filebeat}} configuration.
...
For more on defining processors, refer to [define processors](docs-content://docs/reference/ingestion-tools/fleet/agent-processors.md).
...
For logs that with an existing field being used to represent the service name, map that field to the `service.name` field using the [alias field type](elasticsearch://docs/reference/elasticsearch/mapping-reference/field-alias.md). Follow these steps to update your mapping:
