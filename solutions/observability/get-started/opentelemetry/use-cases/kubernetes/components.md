...
All signals including logs, metrics, and traces are processed by the collectors and sent directly to {{es}} using the [Elasticsearch exporter](elastic-agent://reference/otel-collector/components/elasticsearchexporter.md). A collector's processor pipeline replaces the traditional APM server functionality for handling application traces.
