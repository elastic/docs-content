...
Without leaving the **Hosts** page, you can view enhanced metrics relating to each host running in your infrastructure. In the list of hosts, find the host you want to monitor, then select the **View details** icon ![expand icon](/solutions/images/serverless-expand-icon.png "") (or **Toggle dialog with details** in earlier {{stack}} versions) to display the host details overlay.
