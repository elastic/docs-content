...
Crossing the warning threshold triggers the **Warning** action group; crossing the critical threshold triggers the **Alert** action group. The alert's severity matches whichever threshold triggered it (`warning` or `critical`), and you can use those values in [per-alert snooze](/explore-analyze/alerting/alerts/view-alerts.md) conditions to automatically unsnooze an alert when its severity changes. {applies_to}`stack: ga 9.5+` {applies_to}`serverless: ga`

When you select **Alert me if there’s no data**, the rule triggers if the metrics don’t report any data over the expected time period, or if the rule fails to query {{es}}. {applies_to}`stack: ga =9.0, removed 9.1+` {applies_to}`serverless: unavailable` 
