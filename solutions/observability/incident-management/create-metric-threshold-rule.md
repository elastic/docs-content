...
{applies_to}`stack: ga 9.5+` {applies_to}`serverless: ga` When the warning threshold is met, the alert severity is set to `warning`. When the critical threshold is met, the alert severity is set to `critical`. 
