...
* Use [CSV](elasticsearch://reference/enrich-processor/csv-processor.md) together with [explicit mapping](/manage-data/data-store/mapping/explicit-mapping.md) to index CSV files with geo data. Kibana’s [Import CSV](/explore-analyze/visualize/maps/import-geospatial-data.md) feature can help with this.
...
* Use [geo_match enrich policy](/manage-data/ingest/transform-enrich/example-enrich-data-based-on-geolocation.md) for reverse geocoding. For example, use [reverse geocoding](/explore-analyze/visualize/maps/reverse-geocoding-tutorial.md) to visualize metropolitan areas by web traffic.
...
* [Geo-line aggregation](elasticsearch://reference/aggregations/search-aggregations-metrics-geo-line.md) aggregates all geo_point values within a bucket into a LineString ordered by the chosen sort field. Use geo_line aggregation to create [vehicle tracks](/explore-analyze/visualize/maps/asset-tracking-tutorial.md).
...
Visualize geo data with [Kibana](/explore-analyze/visualize/maps.md). Add your map to a [dashboard](/explore-analyze/dashboards.md) to view your data from all angles.
...
Put machine learning to work for you and find the data that should stand out with anomaly detections. Find credit card transactions that occur in an unusual locations or a web request that has an unusual source location. [Location-based anomaly detections](/explore-analyze/machine-learning/anomaly-detection/geographic-anomalies.md) make it easy to find and explore and compare anomalies with their typical locations.


## Alerting [geospatial-alerting]

Let your location data drive insights and action with [geographic alerts](/explore-analyze/alerting/kibana-alerting-v1/geo-alerting-v1.md). Commonly referred to as geo-fencing, track moving objects as they enter or exit a boundary to receive notifications through common business systems (email, Slack, Teams, PagerDuty, and more).

Interested in learning more? Follow [step-by-step instructions](/explore-analyze/visualize/maps/asset-tracking-tutorial.md) for setting up tracking containment alerts to monitor moving vehicles.
