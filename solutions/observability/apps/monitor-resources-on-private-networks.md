...
* **Set up {{fleet-server}}**: If you are using {{ecloud}}, {{fleet-server}} will already be provided and you can skip this step. To learn more, refer to [Set up {{fleet-server}}](docs-content://docs/reference/ingestion-tools/fleet/fleet-server.md).
* **Create an agent policy**: For more information on agent policies and creating them, refer to [{{agent}} policy](docs-content://docs/reference/ingestion-tools/fleet/agent-policy.md#create-a-policy).
...
Then enroll and run an {{agent}}. You’ll need an enrollment token and the URL of the {{fleet-server}}. You can use the default enrollment token for your policy or create new policies and [enrollment tokens](docs-content://docs/reference/ingestion-tools/fleet/fleet-enrollment-tokens.md) as needed.

For more information on running {{agent}} with Docker, refer to [Run {{agent}} in a container](docs-content://docs/reference/ingestion-tools/fleet/elastic-agent-container.md).
...
::::{note}
You may need to set other environment variables. Learn how in [{{agent}} environment variables guide](docs-content://docs/reference/ingestion-tools/fleet/agent-environment-variables.md).
