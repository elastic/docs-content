...
{{product.apm}} index settings are [{{kib}} Spaces](/deploy-manage/manage-spaces.md)-aware — changes apply only to the currently enabled space. For an example, see [Separate staging and production data](#apm-spaces-example).
...
Multiple APM integration instances is not required though. The simplest way to take advantage of this feature is by creating filtered aliases. See [Separate staging and production data](#apm-spaces-example) for more information.
