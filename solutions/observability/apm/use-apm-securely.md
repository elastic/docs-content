...
| **Is FIPS compatibility available for APM Server?** | Yes! FIPS compatible binaries are available for download. Look for the `Linux x86_64 (FIPS)` or `Linux aarch64 (FIPS)` platform option on the [APM Server download](https://www.elastic.co/downloads/apm) page.<br> Get more details about FIPS compatibility for APM Server and other ingest tools in [FIPS mode for Ingest tools](/deploy-manage/security/fips-ingest.md).|
