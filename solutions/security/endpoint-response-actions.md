...
::::{note}
The default file size maximum is 25 MB, configurable in [`kibana.yml`](/deploy-manage/stack-settings.md) with the `xpack.securitySolution.maxUploadResponseActionFileBytes` setting. You must enter the value in bytes ({applies_to}`stack: removed 9.3`{applies_to}`serverless: removed` the maximum is `104857600` bytes, or 100 MB).
::::
