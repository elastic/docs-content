...
* In addition to the privilege for each response action, users need at least **Read** access to the **[Response Actions History](/solutions/security/configure-elastic-defend/elastic-defend-feature-privileges.md)** privilege to view command output and status in the response console. Without it, running a response action in the console can return an error even when the user has privileges for that action.
::::
