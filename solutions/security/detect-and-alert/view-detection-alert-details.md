...
% - [ ] ./raw-migrated-files/security-docs/security/view-alert-details.md
...
The right panel provides an overview of the alert. Expand any of the collapsed sections to learn more about the alert. You can also hover over fields on the **Overview** and **Table** tabs to display available [inline actions](../get-started/elastic-security-ui.md#inline-actions).
...
* **Investigation guide**: The **Show investigation guide** button displays if the rule associated with the alert has an investigation guide. Click the button to open the expanded Investigation view in the left panel.
    ...
    ::::{tip}
    Add an [investigation guide](launch-timeline-from-investigation-guides.md#add-ig-actions-rule) to a rule when creating a new custom rule or modifying an existing custom rule’s settings.
    ::::

* **Highlighted fields**: Shows relevant fields for the alert and any [custom highlighted fields](create-detection-rule.md#rule-ui-advanced-params) you added to the rule. Custom highlighted fields with values are added to this section. Those without values aren’t added.
...
To use the **Visualize** tab, you must turn on the `securitySolution:enableVisualizationsInFlyout` [advanced setting](../get-started/configure-advanced-settings.md#visualizations-in-flyout).
...
The Insights section is located on the **Overview** tab in the right panel. It offers different perspectives from which you can assess the alert. Click **Insights** to display overviews for [related entities](#entities-overview), [threat intelligence](#threat-intelligence-overview), [correlated data](#correlations-overview), and [host and user prevalence](#prevalence-overview).
...
* **Alerts related by session**: Shows alerts generated during the same [session](../investigate/session-view.md). These alerts share the same session ID, which is a unique ID for tracking a given Linux session. To use this feature, you must enable the **Collect session data** setting in your {{elastic-defend}} integration policy. Refer to [Enable Session View data](../investigate/session-view.md#enable-session-view) for more information.
...
The **Response** section is located on the **Overview** tab in the right panel. It shows [response actions](create-detection-rule.md) that were added to the rule associated with the alert. Click **Response** to display the response action’s results in the left panel.
...
::::{tip}
Go to the **Notes** [page](../investigate/notes.md#manage-notes) to find notes that were added to other alerts.
::::
