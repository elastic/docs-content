...
::::{tab-set}
:::{tab-item} {{stack}} 9.4+
The engine automatically adds `METADATA _id` to the `FROM` command when your query omits it, and makes a best-effort attempt to keep `_id` available in the pipeline (for example by appending `_id` to restrictive `KEEP` commands). Injection happens **when the rule runs** (and during query validation); the **saved rule text** in the editor stays what you entered. You only see `METADATA _id` in the query field if you add it yourself. You do not need to type `METADATA _id` manually for deduplication, including any queries generated by AI-assisted tools.

If `_id` is still missing from the query results (for example after `DROP _id`, `RENAME _id AS …`, or `EVAL _id = …`) the rule editor shows a validation warning. You can save the rule after confirming the **Save with errors** dialog. At execution time, the engine might log warnings and you might get duplicate alerts until you adjust the query to return `_id`.
...
Adding `METADATA _id, _index, _version` enables [alert deduplication](#esql-alert-deduplication). In {{stack}} 9.4 and later, `METADATA _id` is added automatically when omitted; earlier releases require you to add `METADATA _id` manually. Without `_id` in the results, the same source event can generate duplicate alerts across executions.

### Alert deduplication [alert-deduplication]

For non-aggregating queries, include `_id` in the result—for example by adding `METADATA _id` after `FROM` (automatic in {{stack}} 9.4+ when missing), or `METADATA _id, _index, _version` if you need more metadata fields:
...
Ensure you do not `DROP` or filter out `_id` (or rename or overwrite it) in subsequent pipeline steps, or deduplication may fail.
...
| Field | Value | Purpose |
|---|---|---|
| `query` | `FROM ... \| WHERE ... \| LIMIT 100` | A non-aggregating query. Each matching row becomes an alert. For deduplication across executions, `_id` must appear in the results—in {{stack}} 9.4+, `METADATA _id` is added automatically when missing. In earlier versions, include `METADATA _id` (and optionally other metadata fields) after `FROM`. |
