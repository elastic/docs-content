...
::::{tab-set}
:::{tab-item} {{stack}} 9.4+
The detection engine automatically adds `METADATA _id` to the `FROM` command when your query omits it, and makes a best-effort attempt to keep `_id` available in the pipeline (for example by appending `_id` to restrictive `KEEP` commands). Injection happens when the rule runs and during query validation. You do not need to type `METADATA _id` manually for deduplication, including any queries generated by AI-assisted tools.

If `_id` is still missing from the query results (for example after `DROP _id`, `RENAME _id AS …`, or `EVAL _id = …`) the query editor shows a validation warning. You can save the rule after confirming the **Save with errors** dialog. At execution time, the engine might log warnings and you might get duplicate alerts until you adjust the query to return `_id`.
