...
Value lists are lists of items with the same {{es}} [data type](elasticsearch://docs/reference/elasticsearch/mapping-reference/field-data-types.md). You can create value lists with these types:

* `Keywords` (many [ECS fields](ecs://docs/reference/ecs/ecs-field-reference.md) are keywords)
* `IP Addresses`
* `IP Ranges`
* `Text`
