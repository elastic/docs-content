...
| {applies_to}`{ "stack": "ga 9.4+", "serverless": "ga" }` See how many rules succeeded, failed, or have gaps | [Rule Monitoring Overview panel](#rule-monitoring-overview) |
...
### Rule Monitoring Overview panel [rule-monitoring-overview]
```{applies_to}
stack: ga 9.4+
serverless: ga
```

The **Rule Monitoring Overview** panel above the Rules table summarizes how every rule in the space last ran and how much gap time they've accumulated. Refer to [Rule Monitoring tab](/solutions/security/detect-and-alert/fill-rule-gaps.md#rule-monitoring-tab-gaps).
