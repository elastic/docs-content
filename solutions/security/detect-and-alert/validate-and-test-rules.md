...
{applies_to}`stack: ga 9.4` {applies_to}`serverless: ga` With [Agent Builder](/explore-analyze/ai-features/ai-chat-experiences/ai-agent-or-ai-assistant.md), you can [attach a detection rule to the agent](/solutions/security/ai/agent-builder/agent-builder.md#attach-a-detection-rule-to-the-ai-agent) to provide the full rule definition as context. This removes the need to copy and paste and gives the agent access to all rule fields when answering your questions.
