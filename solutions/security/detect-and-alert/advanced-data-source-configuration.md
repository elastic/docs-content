...
:   Relevant if you use {{cps}} to query data across linked {{serverless-short}} projects. Explains which projects a rule searches, where its alerts are stored, which API key a rule needs to reach linked projects, and how the alert limit applies across them. {{serverless-short}} only.
