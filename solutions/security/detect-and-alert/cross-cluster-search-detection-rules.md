...
2. On both the local and remote clusters, [create a role for {{ccs}} privileges](/explore-analyze/cross-cluster-search.md#configure-privileges-for-ccs), and make sure the two roles have *identical* names. Assign each role the following privileges:
