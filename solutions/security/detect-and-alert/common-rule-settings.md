...
:   Specify the maximum number of alerts the rule can create each time it executes. Default is 100.
    ...
    Although it is technically possible to increase `xpack.alerting.rules.run.alerts.max` above its default of `1000`, doing so is not recommended and not supported. Carefully evaluate the potential performance impact on your deployment before making this change.
    ::::
