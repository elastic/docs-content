...
:   Specify the maximum number of alerts the rule can create each time it executes. Default is 100.
    ...
    {applies_to}`serverless: preview` When {{cps}} is enabled, this limit is shared across every project the rule searched, not applied separately to each project. Which projects the rule searches in that run depends on the [{{cps}} scope for that rule](/explore-analyze/alerting/alerts/create-manage-rules.md#cps-scope-for-rules). The limit still applies once. Refer to [{{cps-cap}} and detection rules](/solutions/security/detect-and-alert/cross-project-search-detection-rules.md#cps-rules-max-alerts).
