...
% - [ ] ./raw-migrated-files/security-docs/security/tuning-detection-signals.md
...
* Add [exceptions](add-manage-exceptions.md) to detection rules.
...
* [Clone and modify](manage-detection-rules.md#manage-rules-ui) detection rule queries so they are aligned with local policy exceptions. This reduces noise while retaining actionable alerts.
...
* Enable [alert suppression](suppress-detection-alerts.md) for custom query rules to reduce the number of repeated or duplicate alerts.
...
* Add an exception to the rules that [exclude specific processes](#filter-rule-process). For example, `process.name is <process-name>`.
...
::::{note}
{{elastic-sec}} provides limited support for indicator match rules. Visit [support limitations](../detect-and-alert.md#support-indicator-rules) for more information.
::::
...
::::{tip}
If your organization is widely distributed and the workforce travels a lot, use the `windows_anomalous_user_name_ecs`, `linux_anomalous_user_name_ecs`, and `suspicious_login_activity_ecs` [{{ml}}](../advanced-entity-analytics/anomaly-detection.md) jobs to detect suspicious authentication activity.
::::
