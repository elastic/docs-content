...
% - [ ] ./raw-migrated-files/security-docs/security/alert-suppression.md
...
* {{ml-cap}} rules have [additional requirements](../advanced-entity-analytics/machine-learning-job-rule-requirements.md) for alert suppression.
...
* [Custom query](create-detection-rule.md#create-custom-rule)
* [Threshold](create-detection-rule.md#create-threshold-rule)
* [Indicator match](create-detection-rule.md#create-indicator-rule)
* [Event correlation](create-detection-rule.md#create-eql-rule)
* [New terms](create-detection-rule.md#create-new-terms-rule)
* [{{esql}}](create-detection-rule.md#create-esql-rule)
* [{{ml-cap}}](create-detection-rule.md#create-ml-rule)
...
You can configure alert suppression when you create or edit a supported rule type. Refer to documentation for creating [custom query](create-detection-rule.md#create-custom-rule), [threshold](create-detection-rule.md#create-threshold-rule), [event correlation](create-detection-rule.md#create-eql-rule), [new terms](create-detection-rule.md#create-new-terms-rule), [{{esql}}](create-detection-rule.md#create-esql-rule), or [{{ml}}](create-detection-rule.md#create-ml-rule) rules for detailed instructions.
...
* If a rule times out while suppression is turned on, try shortening the rule’s [look-back](create-detection-rule.md#rule-schedule) time or turn off suppression to improve the rule’s performance.
...
* **Threshold, event correlation, {{esql}}, and {{ml}}:** The maximum number of alerts is the value you choose for the rule’s **Max alerts per run** [advanced setting](create-detection-rule.md#rule-ui-advanced-params), which is `100` by default.
* **Indicator match and new terms:** The maximum number is five times the value you choose for the rule’s **Max alerts per run** [advanced setting](create-detection-rule.md#rule-ui-advanced-params). The default value is `100`, which means the default maximum limit for indicator match rules and new term rules is `500`.
