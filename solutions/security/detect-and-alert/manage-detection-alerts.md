...
::::{tip}
When you send an alert generated by a [threshold rule](/solutions/security/detect-and-alert/using-the-rule-builder.md) to Timeline, all matching events are listed in the Timeline, even ones that did not reach the threshold value. For example, if you have an alert generated by a threshold rule that detects 10 failed login attempts, when you send that alert to Timeline, all failed login attempts detected by the rule are listed.
::::
...
::::{note}
Refer to [Timeline](/solutions/security/investigate/timeline.md) for information on creating Timelines and Timeline templates. For information on how to add Timeline templates to rules, refer to [Create a detection rule](/solutions/security/detect-and-alert/using-the-rule-builder.md).
::::
