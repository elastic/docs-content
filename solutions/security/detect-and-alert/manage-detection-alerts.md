...
* View an alert’s details. Click the **View details** button from the Alerts table to open the alert details flyout. Learn more at [View detection alert details](view-detection-alert-details.md).
...
* View the details of the host and user associated with the alert. In the Alerts table, click a host name to open the [host details flyout](../explore/hosts-page.md#host-details-flyout), or a user name to open the [user details flyout](../explore/users-page.md#user-details-flyout).
...
* Use the drop-down filter controls to filter alerts by up to four fields. By default, you can filter alerts by **Status**, **Severity**, **User**, and **Host**, and you can [edit the controls](#drop-down-filter-controls) to use other fields.
...
* Hover over a value to display available [inline actions](../get-started/elastic-security-ui.md#inline-actions), such as **Filter In**, **Filter Out**, and **Add to timeline**. Click the expand icon for more options, including **Show top *x*** and **Copy to Clipboard**. The available options vary based on the type of data.
...
1. Click the three-dot icon next to the controls (![Three-dot icon](../../../images/security-three-dot-icon-blue.png "title =20x20")), then select **Edit Controls**.
2. Do any of the following:

    * To reorder controls, click and drag a control by its handle (![Control handle](../../../images/security-handle-icon.png "title =20x20")).
    * To remove a control, hover over it and select **Remove control** (![Red X icon](../../../images/security-red-x-icon.png "title =20x20")).
    * To edit a control, hover over it and select **Edit control** (![Edit icon](../../../images/security-pencil-icon.png "title =20x20")).
    * To add a new control, click **Add Controls** (![Add icon](../../../images/security-add-icon-blue.png "title =20x20")). If you already have four controls, you must first remove one to make room for the new one.
...
4. Click **Save pending changes** (![Save icon](../../../images/security-save-icon-blue.png "title =20x20")).
...
* Select the **Take actions** menu to perform a bulk action on all alerts in a group, such as [changing their status](#detection-alert-status).
* Click a group’s name or the expand icon (![Grouped alerts expand icon](../../../images/security-expand-icon-vertical-right.png "title =20x20")) to display alerts within that group. You can filter and customize this view like any other alerts table.
...
* **Fields**: Select the fields to display in the table. You can also add [runtime fields](../get-started/create-runtime-fields-in-elastic-security.md) to detection alerts and display them in the Alerts table.
...
* [Add detection alerts to cases](add-detection-alerts-to-cases.md)
...
* [Add an endpoint exception from an alert](add-manage-exceptions.md#endpoint-rule-exceptions)
* [Isolate an alert’s host](../endpoint-response-actions/isolate-host.md)
* [Perform response actions on an alert’s host](../endpoint-response-actions.md) (Alert details flyout only)
* [Run Osquery against an alert](../investigate/run-osquery-from-alerts.md)
* [View alerts in Timeline](#signals-to-timelines)
* [Visually analyze an alert’s process relationships](../investigate/visual-event-analyzer.md)
* [Add notes to alerts](../investigate/notes.md#notes-alerts-events)
...
* [beta] To bulk-change the status of [grouped alerts](#group-alerts), select the **Take actions** menu for the group, then select a status.
* In an alert’s details flyout, click **Take action** and select a status.


### Apply and filter alert tags [apply-alert-tags]

Use alert tags to organize related alerts into categories that you can filter and group. For example, use the `False Positive` alert tag to label a group of alerts as false positives. Then, search for them by entering the `kibana.alert.workflow_tags : "False Positive"` query into the KQL bar. Alternatively, use the Alert table’s [drop-down filters](#drop-down-filter-controls) to filter for tagged alerts.

::::{note}
You can manage alert tag options by updating the `securitySolution:alertTags` advanced setting. Refer to [Manage alert tag options](../get-started/configure-advanced-settings.md#manage-alert-tags) for more information.
::::
...
For information about exceptions and how to use them, refer to [Add and manage exceptions](add-manage-exceptions.md).
...
::::{tip}
When you send an alert generated by a [threshold rule](create-detection-rule.md) to Timeline, all matching events are listed in the Timeline, even ones that did not reach the threshold value. For example, if you have an alert generated by a threshold rule that detects 10 failed login attempts, when you send that alert to Timeline, all failed login attempts detected by the rule are listed.
::::
...
::::{note}
Refer to [*Timeline*](../investigate/timeline.md) for information on creating Timelines and Timeline templates. For information on how to add Timeline templates to rules, refer to [*Create a detection rule*](create-detection-rule.md).
::::
