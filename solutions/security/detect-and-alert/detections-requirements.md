...
| Manage value lists.<br><br>Create the `.lists` and `.items` data streams in your space<br><br>**NOTE**: To initiate the process that creates the data streams, you must visit the Rules page for each appropriate space. | `manage` | `manage`, `write`, `read`, and `view_index_metadata` for these data streams, where `<space-id>` is the space name:<br><br>- `.lists-<space-id>`<br>- `.items-<space-id>`<br> | * {applies_to}`stack ga: 9.0`: `All` for the `Security` and `Saved Objects Management` features<br><br>* {applies_to}`stack ga: 9.3` {applies_to}`serverless: ga`: `All` for the `Rules` and `Saved Objects Management` features |
