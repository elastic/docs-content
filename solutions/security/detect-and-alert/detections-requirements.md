...
To use the [Detections feature](../detect-and-alert.md), you first need to configure a few settings. You also need the [appropriate license](https://www.elastic.co/subscriptions) to send [notifications](create-detection-rule.md#rule-notifications) when detection alerts are generated.

::::{important}
Several steps are **only** required for **self-managed** {{stack}} deployments. If you’re using an Elastic Cloud deployment, you only need to [enable detections](#enable-detections-ui).
::::


Additionally, there are some [advanced settings](#adv-list-settings) used to configure {{kib}} [value list](create-manage-value-lists.md) upload limits.
...
You can set limits to the number of bytes and the buffer size used to upload [value lists](create-manage-value-lists.md) to {{elastic-sec}}.
