...
:::{note}
{applies_to}`stack: ga 9.1` {applies_to}`stack: ga 8.19` {applies_to}`serverless: ga` The agent has access to index names and field metadata from your cluster. This contextual information helps improve ES|QL generation, though it may slightly increase response times.
:::
