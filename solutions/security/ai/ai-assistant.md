...
:::{note}
{applies_to}`stack: ga 9.1` {applies_to}`serverless: ga` The agent has access to index names and field metadata from your cluster. This contextual information is used to improve {{esql}} generation, and may result in slightly longer response times.
:::
