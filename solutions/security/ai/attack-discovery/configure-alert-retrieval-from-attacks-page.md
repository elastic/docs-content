...
* If your subscription or project includes [Elastic Managed LLMs](kibana://reference/connectors-kibana/elastic-managed-llm.md), one may already be available with no setup required. Otherwise, add a connector from the dropdown. Refer to [Configure access to LLMs](/explore-analyze/ai-features/llm-guides/llm-connectors.md) to choose and set one up.
