...
## Ways to run Attack Discovery [attack-discovery-trigger-paths]
...
Every attack discovery, however it's triggered, runs through the same audited pipeline. You can start a run:

- **Manually**—Click **Run** on this page. Refer to [Generate discoveries manually](#attack-discovery-generate-discoveries).
- **On a schedule**—Refer to [Schedule discoveries](#schedule-discoveries).
- **From a workflow**—Embed Attack Discovery as a step in an [Elastic Workflow](/explore-analyze/workflows.md), for example to trigger a run when an alert threshold is met. Refer to [Automate Attack Discovery](/solutions/security/ai/attack-discovery-automation.md).
- **From a chat**—Summon Attack Discovery from an {{agent-builder}} conversation. Refer to [Automate Attack Discovery](/solutions/security/ai/attack-discovery-automation.md).
...
Attack Discovery is skill-augmented by default: every run is enriched by the **`attack-discovery-generator`** skill, which cross-references host, user, and time context to add related alerts your query might have missed, corroborates evidence from other security AI skills, and searches raw endpoint logs for additional context.

You control which alerts are analyzed using three independent toggles. Each toggle is additive—it contributes alerts to a merged candidate set on top of the others:

| Toggle | Default | What it does |
|---|---|---|
| **Attack discovery skill retrieves alerts** | On | The skill ground-truths and augments your alert set using its own retrieval logic. |
| **ES\|QL or custom query** | Off | Scope alerts using an ES\|QL query or the query builder—this replaces the KQL query and **Number of alerts** slider from earlier versions. |
| **Alert retrieval workflows** | Off | Run one or more of your own alert-retrieval [Elastic Workflows](/explore-analyze/workflows.md) to supply alerts. |

Under **Alert summary** you can view a summary of the selected alerts grouped by various fields, and under **Alerts preview** you can view more details about the selected alerts.

:::{important}
Turning off **Attack discovery skill retrieves alerts** doesn't disable the skill entirely. It still enriches, validates, and may curate the alerts you provide, and still searches raw logs for additional context. The toggle only controls whether the skill adds *extra* alerts beyond what your query or workflows already supply.
:::

This behavior is available behind the `securitySolution.attackDiscoveryWorkflowsEnabled` advanced setting, which is off by default. If you already use Attack Discovery, enabling it doesn't disrupt existing schedules—they keep running on the previous retrieval path until you migrate them. Refer to [Automate Attack Discovery](/solutions/security/ai/attack-discovery-automation.md) for how to migrate.
...
:::{applies-item} { "stack": "ga 9.0-9.4", "serverless": "unavailable" }
...
Click a discovery's title to open its flyout, which organizes discovery information into an **Overview** tab:

- **AI Summary**—a descriptive title and summary of the potential threat, generated by the connected LLM.
- **Visualizations**—an accordion containing the **Attack Chain** diagram, which maps the discovery's associated alerts to the parts of the [MITRE ATT&CK matrix](https://attack.mitre.org/) they correspond to. The accordion's expanded/collapsed state is remembered across discoveries.
- **Insights**—the implicated entities (users and hosts) involved in the discovery, and the number of correlated alerts.

<!-- TODO(writer): capture a 9.5+ screenshot of the flyout's Overview tab (AI Summary, Visualizations accordion, Insights section) before publishing—the existing security-attck-disc-example-disc.png shows the pre-9.5 expand/collapse list and shouldn't be reused here. -->
...
{applies_to}`stack: ga 9.5+` {applies_to}`serverless: ga` A scheduled run is skill-augmented like any other—every execution is enriched and validated before discoveries are persisted, and notifications route to any configured Alerting Framework connector. Refer to [Automate Attack Discovery](/solutions/security/ai/attack-discovery-automation.md#schedule-ad) for the always-on automation story.
