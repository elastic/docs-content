...
:::{note}
By default, the index selector excludes dot-prefixed indices and index patterns that are part of the [Security default data view](/solutions/security/workspace-settings/data-views-elastic-security.md#default-data-view-security) (such as `logs-*`). This prevents adding excessive data to privileged user monitoring, which could cause performance issues.
:::
