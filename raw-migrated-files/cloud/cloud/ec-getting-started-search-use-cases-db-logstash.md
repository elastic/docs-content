...
This guide explains how to ingest data from a relational database into Elasticsearch Service through [Logstash](logstash://docs/reference/ingestion-tools/logstash/index.md), using the Logstash [JDBC input plugin](logstash://docs/reference/ingestion-tools/logstash/plugins-inputs-jdbc.md). It demonstrates how Logstash can be used to efficiently copy records and to receive updates from a relational database, and then send them into {{es}} in an Elasticsearch Service deployment.
