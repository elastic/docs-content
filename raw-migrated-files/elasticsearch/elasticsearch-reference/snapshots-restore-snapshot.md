...
* To restore a snapshot, the cluster’s global metadata must be writable. Ensure there aren’t any [cluster blocks](elasticsearch://docs/reference/elasticsearch/configuration-reference/miscellaneous-cluster-settings.md#cluster-read-only) that prevent writes. The restore operation ignores [index blocks](elasticsearch://docs/reference/elasticsearch/index-settings/index-block.md).
