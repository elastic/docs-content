...
To define users, the {{security-features}} provide the [users](elasticsearch://docs/reference/elasticsearch/command-line-tools/users-command.md) command-line tool. This tool enables you to add and remove users, assign user roles, and manage user passwords.
...
3. Add user information to the `ES_PATH_CONF/users` file on each node in the cluster.
    ...
    While it is possible to modify the `users` files directly using any standard text editor, we strongly recommend using the [*elasticsearch-users*](elasticsearch://docs/reference/elasticsearch/command-line-tools/users-command.md) tool to apply the required changes.
...
4. Add role information to the `ES_PATH_CONF/users_roles` file on each node in the cluster.
    ...
    You can use the [*elasticsearch-users*](elasticsearch://docs/reference/elasticsearch/command-line-tools/users-command.md) tool to update this file. You must ensure that the same changes are made on every node in the cluster.
