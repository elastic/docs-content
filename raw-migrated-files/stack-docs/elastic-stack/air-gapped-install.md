...
* To be able to use the GeoIP processor, refer to [the GeoIP processor documentation](elasticsearch://docs/reference/ingestion-tools/enrich-processor/geoip-processor.md#manually-update-geoip-databases) for instructions on downloading and deploying the required databases.
...
Elastic {{beats}} are light-weight data shippers. They do not require any unique setup in the air-gapped scenario. To learn more, refer to the [{{beats}} documentation](beats://docs/reference/ingestion-tools/index.md).


#### 1.4. {{ls}} [air-gapped-logstash]

{{ls}} is a versatile data shipping and processing application. It does not require any unique setup in the air-gapped scenario. To learn more, refer to the [{{ls}} documentation](logstash://docs/reference/ingestion-tools/logstash/index.md).
...
To learn more about install and configuration, refer to the [{{agent}} install documentation](docs-content://docs/reference/ingestion-tools/fleet/install-elastic-agents.md). Make sure to check the requirements specific to running {{agents}} in an [air-gapped environment](docs-content://docs/reference/ingestion-tools/fleet/air-gapped.md).
...
To learn more about installing {{fleet-server}}, refer to the [{{fleet-server}} set up documentation](docs-content://docs/reference/ingestion-tools/fleet/fleet-server.md).
...
* [Install trained models in an air-gapped environment with Eland](eland://docs/reference/elasticsearch/elasticsearch-client-eland/machine-learning.md#ml-nlp-pytorch-air-gapped).


#### 1.13 {{kib}} Product documentation for AI Assistants [air-gapped-kibana-product-documentation]

Detailed install and configuration instructions are available in the [{{kib}} AI Assistants settings documentation](kibana://docs/reference/configuration-reference/ai-assistant-settings.md).
...
The container image can be downloaded from the official Elastic Docker repository, as described in the {{fleet}} and {{elastic-agent}} [air-gapped environments](docs-content://docs/reference/ingestion-tools/fleet/air-gapped.md) documentation.
...
**Example:** [Fleet settings in {{kib}}](kibana://docs/reference/configuration-reference/fleet-settings.md)
...
:   Takes a list of {{agent}} policies in the format expected by the [{{kib}} {{fleet}} HTTP API](docs-content://docs/reference/ingestion-tools/fleet/fleet-api-docs.md). Refer to the setting in [Preconfiguration settings](kibana://docs/reference/configuration-reference/fleet-settings.md#_preconfiguration_settings_for_advanced_use_cases) for the format. See also [D.2.3. Using the {{kib}} {{fleet}} API](../../../deploy-manage/deploy/self-managed/air-gapped-install.md#air-gapped-agent-integration-configure-fleet-api).
...
Other settings
:   You can add other, more discretionary settings for {{fleet}}, {{agents}}, & policies. Refer to [Fleet settings in {{kib}}](kibana://docs/reference/configuration-reference/fleet-settings.md).
