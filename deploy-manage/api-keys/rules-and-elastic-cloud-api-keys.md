...
| Key management | {{kib}} API keys page | {{ecloud}} Console (organization owners only) |
...
- **Organizational changes and orphaned rules**: {{ecloud}} API keys do not automatically follow every organizational change. If the key owner no longer exists in the organization, affected rules become orphaned and require manual intervention to re-bind them to a valid owner and key.
...
- **Resolve migration errors promptly**: If the UI shows API key errors for a rule, open the rule and save it with a valid user following the in-product prompts. Rules with missing owners or invalid role assignments will not run until resolved.
...
- **When editing rules, be aware that ownership no longer transfers**: Unlike {{es}} API keys, the key stays tied to the original owner when a different user edits a rule. Privileges do not change on edit, but this also means any access issues from migration will not resolve themselves when a rule is updated.

- **For new {{serverless-short}} rules, use {{ecloud}} API keys**: Do not rely on {{es}} API key behavior for new rules in {{serverless-short}} projects.
