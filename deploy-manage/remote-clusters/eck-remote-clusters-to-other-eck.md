...
3. **Configure the {{es}} resource**
    ...
    * Load the API key from the previously created secret using [`secureSettings`](/deploy-manage/security/k8s-secure-settings.md)
    * Mount the CA certificate from the previously created ConfigMap [as a custom file](/deploy-manage/plugins-and-configuration-files/cloud-on-k8s/custom-configuration-files-plugins.md#use-a-volume-and-volume-mount-together-with-a-configmap-or-secret) in the {{es}} Pods
