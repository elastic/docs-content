...
```sh
curl -k -X GET -H "Authorization: ApiKey $ECE_API_KEY" <COORDINATOR_HOST_URL>:12443//api/v1/regions/ece-region/platform/configuration/trust-relationships?include_certificate=false
```
...
```sh
curl -k -H 'Content-Type: application/json' -X PUT -H "Authorization: ApiKey $ECE_API_KEY" <COORDINATOR_HOST_URL>:12443/api/v1/deployments/$DEPLOYMENT_ID/elasticsearch/$REF_ID/remote-clusters -d '
{
  "resources" : [
    {
...
```sh
curl -k -X GET -H "Authorization: ApiKey $ECE_API_KEY" <COORDINATOR_HOST_URL>:12443/api/v1/deployments/$DEPLOYMENT_ID/elasticsearch/$REF_ID/remote-clusters
```
