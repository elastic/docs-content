...
Consult your MCP host's documentation for how to clear cached OAuth credentials and force a new authorization. Most hosts maintain only one connection for each MCP server URL, so reconfiguring with the same URL reuses the existing connection unless the cached credentials are cleared. For hosts that use `mcp-remote`, delete `~/.mcp-auth` to clear stored tokens.
