...
```sh
... Exporting failed. Dropping data. ... no more retries left: failed to make an HTTP request: Post \"https://otel-auto-ops.us-east-2.aws.svc.elastic.cloud:443/v1/logs\": ...
```
...
```json
curl -XPOST -i \
https://otel-auto-ops.${region}.${csp}.svc.elastic.cloud:443/v1/logs \
-H 'Content-Type: application/json'
...
```json
  POST https://otel-auto-ops.${region}.${csp}.svc.elastic.cloud:443/v1/logs
```
:::{note}
If you are using Docker, you might need to complete this configuration directly using the `HTTP_PROXY`, `HTTPS_PROXY`, and `NO_PROXY` environment variables.
:::