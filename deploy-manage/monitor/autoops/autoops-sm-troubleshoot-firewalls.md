...
| You are using a custom SSL/TLS configuration with {{es}} | Add `--insecure` to your curl command to temporarily disable SSL/TLS certificate verification and test whether a certificate issue is causing the problem. We do not recommend disabling verification in production environments. <br><br> If you are using API key authentication: `curl -XGET -i $AUTOOPS_ES_URL \ -H "Authorization: ApiKey $AUTOOPS_ES_API_KEY"` <br><br> If you are using username/password authentication: `curl -XGET -i $AUTOOPS_ES_URL \ -u $AUTOOPS_ES_USERNAME` <br><br> If the issue is resolved, you need to [configure {{agent}} with your custom SSL/TLS certificate](/deploy-manage/monitor/autoops/autoops-sm-custom-certification.md). If the issue persists, contact [Elastic support](https://support.elastic.co/).| 
