...
* [My `AutoOpsAgentPolicy` entered an `Invalid` phase after a license change.](#invalid-phase-license-change) {applies_to}`eck: ga 3.3.1`
...
$$$invalid-phase-license-change$$$**My `AutoOpsAgentPolicy` entered an `Invalid` phase after a license change.** {applies_to}`eck: ga 3.3.1`
:   The minimum AutoOps Agent version depends on your license type. Enterprise licenses support agent versions 9.2.1 and later, while Basic licenses require 9.2.4 or later. If your license is downgraded from Enterprise to Basic and your AutoOps Agent version is between 9.2.1 and 9.2.3, the policy will fail validation and enter an `Invalid` phase.
