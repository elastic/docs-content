...
:   Yes. AutoOps {{agent}} comes bundled with the `autoops_es_debug.yml` configuration file, which you can use to view a sample of the data gathered from your cluster that would be sent to {{ecloud}}.
    ...
    ```
    3. Run the command to start the container. The data sample will appear in the container's standard output as a JSON file.
    4. The data sample will be downloaded to the container's standard output as a JSON file. To view the output, `docker logs autoops-otel-agent` to view the output and i.e. pipe it to `jq` for prettier formatting.
