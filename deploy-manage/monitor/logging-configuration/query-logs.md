...
- `elasticsearch.querylog.params`: Query parameters, in case [parametrized query](elasticsearch://reference/query-languages/esql/esql-rest.md#esql-rest-params) was used. Only supported for `sql` and `esql`. Positional parameters are reported under the field name `?`.  {applies_to}`stack: ga 9.5`
