...
**Configuration files** are files that {{es}} reads at runtime, such as synonym dictionaries, scripts, SAML metadata, GeoIP databases, or certificates. What these are called depends on where you run {{es}}: on {{ech}} and {{ece}} you package them as ZIP archives called *bundles*, on {{eck}} you mount them from ConfigMaps or Secrets, and in self-managed clusters you place them in each node's configuration directory.
