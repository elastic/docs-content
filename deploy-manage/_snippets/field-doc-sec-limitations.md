...
When a user's role enables [document level security](/deploy-manage/users-roles/controlling-access-at-document-field-level.md) for a data stream or index:
...
When a user's role enables document or [field level security](/deploy-manage/users-roles/controlling-access-at-document-field-level.md) for a data stream or index:
...
* The request cache is disabled for search requests if either of the following are true:
    ...
    * The role query that defines document level security is [templated](/deploy-manage/users-roles/controlling-access-at-document-field-level.md#templating-role-query) using a [stored script](/explore-analyze/scripting/modules-scripting-store-and-retrieve.md).
