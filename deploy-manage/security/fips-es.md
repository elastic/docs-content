...
On ECK, deliver the configuration described on this page through custom {{es}} container images or `podTemplate` overrides. For example, use an init container to install the FIPS security provider JARs, and set JVM properties through the `podTemplate` environment or a custom `jvm.options` file.
:::
