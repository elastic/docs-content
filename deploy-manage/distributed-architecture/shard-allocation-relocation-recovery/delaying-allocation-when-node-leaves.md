...
If the node is being [restarted gracefully](https://www.elastic.co/docs/api/doc/elasticsearch/v8/operation/operation-shutdown-put-node), the `allocation_delay` parameter passed in `PUT /_nodes/{node_id}/shutdown` may override this setting if set to a longer value.
