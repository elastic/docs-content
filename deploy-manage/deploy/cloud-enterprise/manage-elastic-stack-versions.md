...
```sh
curl -X GET -u USER:PASSWORD <COORDINATOR_HOST_URL>:12443/api/v1/stack/versions
```

For example (output abridged for brevity):

```
curl -X GET -u admin:4Z52y8Gq7PrxMDy47ipJPSh4ozBMynOGa9HWxcy2D3j <COORDINATOR_HOST_URL>:12443/api/v1/stack/versions
{
  "stacks": [
    {
      "version": "2.4.5",
...
```
You can also query for a specific version with a URI such as `<COORDINATOR_HOST_URL>:12443/api/v1/stack/versions/5.3.2`, for example.
...
2. Add the {{stack}} pack with the following API call:
    ...
    ```sh
    curl -X POST -u USER:PASSWORD <COORDINATOR_HOST_URL>:12443/api/v1/stack/versions \
    ...
    ```sh
    curl -X POST -u admin:pGX5DwKzVAAIeCIpTwwAkCuJDu0ASdFP33UmYpfogfF <COORDINATOR_HOST_URL>:12443/api/v1/stack/versions \
        -H 'content-type: application/zip' \
...
2. Update the {{stack}} pack with the following API call:
    ...
    ```sh
    curl -X PUT -u USER:PASSWORD <COORDINATOR_HOST_URL>:12443/api/v1/stack/versions/VERSION \ <1>
        -H 'content-type: application/zip' \
    ...
    ```sh
    curl -X PUT -u admin:pGX5DwKzVAAIeCIpTwAAkCuJDu0ASdFP33UmYpfogfF <COORDINATOR_HOST_URL>:12443/api/v1/stack/versions/6.4.0 \
        -H 'content-type: application/zip' \
