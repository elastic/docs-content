...
This tutorial is a follow-up to the tutorial [Install a self-managed {{stack}}](tutorial-self-managed-install.md). The installation tutorial describes how to configure a multi-node {{es}} cluster and then set up {{kib}}, followed by {{fleet-server}} and {{agent}}. It relies on the {{es}} [automatic security setup](/deploy-manage/security/self-auto-setup.md) and the self-signed certificate generated by the {{fleet-server}} Quick Start flow, as described in its [security overview](./tutorial-self-managed-install.md#security-overview).
...
The existing `elasticsearch.yml` configuration in all nodes should include settings like:
...
1. When the archive is unpacked, it creates a `ca` directory with the following files:
    ...
    * `ca.key`: The certificate authority's private key.
    ...
    In the next steps, use this CA to generate transport certificates for all {{es}} nodes in the cluster. Keep the `ca.key` file in a secure location and do not distribute it to the nodes.
...
1. Unzip the file:
    ...
    The process generates a structure like this. The `ca/` directory already exists from the previous step, and the node directories are added when you unpack `certificate-bundle.zip`:
...
1. View the files that were unpacked for one node directory:
...
After completing this step, each {{es}} node should have the following files:
...
1. From the host used to generate all certificates, do the following:
...
1. On each {{es}} node, move the copied certificate files from that directory to the configuration directory:
    ...
    ```
    1. For consistency and operational simplicity, the same filename is used for the final certificate file on all {{es}} nodes. You can keep the original filenames (`<es-node-name>.p12`) if you prefer, but make sure to update the configuration accordingly in the next step.
...
1. Import the original CA into the new `transport_cert_new.p12` PKCS#12 file. This step ensures that reconfigured nodes can join and communicate with existing nodes that are still using the previous CA.
...
1. Apply the changes depending on the selected approach:

    - **Rolling restart**: Restart the node, ensure it rejoins the cluster with the new certificates, and then repeat this procedure on the next node.
...
1. Rename the files to use meaningful names:
