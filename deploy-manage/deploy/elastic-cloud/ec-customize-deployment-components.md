...
::::{note}
[Vector search optimized instance configurations](https://www.elastic.co/search-labs/blog/elasticsearch-vector-large-scale-part1) use a lower heap-to-RAM ratio than the one described above, reserving more memory for the page cache instead. To check the actual heap size assigned to your instances, use the [cat nodes API]({{es-apis}}operation/operation-cat-nodes).
::::
