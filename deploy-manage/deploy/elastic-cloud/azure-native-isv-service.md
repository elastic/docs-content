...
:   First create a new account configured with {{ecloud}} Azure Native ISV Service, then perform the migration as follows:
    ...
    6. In the new {{es}} resource, follow the steps in [Restore from a snapshot](/migrate/elasticsearch-data/index.md#ec-restore-snapshots) to register the custom snapshot repository from Step 1.
