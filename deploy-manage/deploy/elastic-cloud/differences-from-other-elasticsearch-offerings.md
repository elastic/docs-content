...
| **[Native realm authentication](/deploy-manage/users-roles/cluster-or-deployment-auth/native.md)** | ✅ | ❌ | {{serverless-short}} does not support {{es}} authentication realms. Project-level access uses API keys, while user authentication (including SAML SSO) is managed at the [organization level](/deploy-manage/users-roles/cloud-organization.md). |
