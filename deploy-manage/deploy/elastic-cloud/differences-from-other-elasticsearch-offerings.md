...
| **Programmatic auth** | ✅ [Elastic API keys](/deploy-manage/api-keys.md) | ✅ [Elastic API keys](/deploy-manage/api-keys.md)<br><br>✅ [Application connections](/deploy-manage/app-connections.md) (Tech preview - MCP clients only) | Available across {{ech}} and Serverless using deployment, project, and {{ecloud}} API key types.<br><br>Serverless also supports [application connections](/deploy-manage/app-connections.md) for clients to authorize through OAuth browser consent. |
