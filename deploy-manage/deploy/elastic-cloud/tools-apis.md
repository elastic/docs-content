...
| Area | API | Tasks |
| --- | --- | --- |
| {{ecloud}} organization<br><br>{{ech}} deployments | [{{ecloud}} API]({{cloud-apis}}) | Manage your Cloud organization, members, costs, billing, and more.<br><br>Manage your hosted deployments and all of the resources associated with them, including scaling or autoscaling resources, and managing network security, deployment extensions, remote clusters, and {{stack}} versions.<br><br>For many common deployment operations, use dedicated endpoints (for example upgrade, tier updates, deployment resource user settings, or snapshot repository attach/detach). Refer to [Manage deployments using the {{ecloud}} API](manage-deployments-using-elastic-cloud-api.md) and the [{{ecloud}} RESTful API](cloud://reference/cloud-hosted/ec-api-restful.md) for usage information and examples. |
