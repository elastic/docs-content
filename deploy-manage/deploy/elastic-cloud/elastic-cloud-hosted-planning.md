...
* **{{kib}} nodes/instances**: For production systems, we recommend that each {{kib}} node/instance in your cluster has [at least 2 GB of RAM](/deploy-manage/upgrade/deployment-or-cluster/upgrade-on-ech.md#increase-your-kib-instance-size). {applies_to}`ess: ga 9.4+` {{kib}} nodes/instances in new {{ech}} deployments have 2 GB of RAM by default. 
