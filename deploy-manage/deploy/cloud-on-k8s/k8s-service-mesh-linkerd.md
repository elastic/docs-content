...
Alternatively, if you only want specific resources to join the mesh, add the `linkerd.io/inject: enabled` annotation to the `podTemplate` (check [API documentation](cloud-on-k8s://docs/reference/cloud/cloud-on-k8s/k8s-api-reference.md)) of the resource as follows:
