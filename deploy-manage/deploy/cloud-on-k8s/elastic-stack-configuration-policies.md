...
### Use variable substitution to share a policy across environments [k8s-stack-config-policy-variable-substitution-example]
```{applies_to}
deployment:
  eck: ga 3.5
```

Store environment-specific values in a ConfigMap and reference them from the policy with `${VAR}`. For full details on syntax, namespace rules, and the `optional` flag, refer to [Variable substitution](#k8s-stack-config-policy-variable-substitution).
...
spec:
  ...
  elasticsearch:
    clusterSettings:
      ...
      indices.breaker.fielddata.limit: "${BREAKER_LIMIT}"
```

Updating the ConfigMap triggers an automatic reconciliation of the policy without modifying the policy resource itself.
