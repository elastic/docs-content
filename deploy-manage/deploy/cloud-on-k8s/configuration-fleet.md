...
To know more about {{fleet}} architecture and related components, check the {{fleet}} [documentation](docs-content://docs/reference/ingestion-tools/fleet/fleet-server.md).
...
Please note that the environment variables related to policy selection mentioned in the {{agent}} [docs](docs-content://docs/reference/ingestion-tools/fleet/agent-environment-variables.md) like `FLEET_SERVER_POLICY_ID` will be managed by the ECK operator.
