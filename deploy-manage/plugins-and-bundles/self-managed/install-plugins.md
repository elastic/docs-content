...
# Install plugins with the elasticsearch-plugin CLI [installation]
...
The `elasticsearch-plugin` tool is located in the `$ES_HOME/bin` directory by default, but it might be in a different location depending on which {{es}} package you installed. For more information, refer to [Find the plugins directory](plugins-directory.md).

To get usage instructions, run the following command:
...
## Install core {{es}} plugins [_core_elasticsearch_plugins]

[Core {{es}} plugins](elasticsearch://reference/elasticsearch-plugins/index.md) can be installed as follows:
...
* [](custom-url.md)
* [](install-multiple-plugins.md)
* [](list-remove-update.md)
* [](other-command-line-parameters.md)
