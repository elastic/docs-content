...
How you manage plugins depends on how you run {{es}}. Use a declarative configuration file with the official Docker image, or the `elasticsearch-plugin` CLI for package and archive installs. To browse available plugins, refer to [{{es}} plugins](elasticsearch://reference/elasticsearch-plugins/index.md).
...
After you install a plugin, restart each node before it becomes available. You can [configure mandatory plugins](mandatory-plugins.md) so a node refuses to start if a critical plugin is missing. For silent, verbose, and related options, refer to [Use other command line parameters](other-command-line-parameters.md).
