...
If you are already running an 8.18.x version, it's also recommended to upgrade to the latest 8.18 patch release before upgrading to 9.x, as that will ensure the latest version of the upgrade assistant will be used, together with other possible bug fixes that could have implications in the upgrade.

Alternatively to upgrading the cluster, you can create a 9.0.0 deployment and reindex from remote. For more information, refer to [Reindex to upgrade](#reindex-to-upgrade).
