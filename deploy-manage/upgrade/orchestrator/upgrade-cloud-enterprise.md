...
* Starting in ECE 3.8.0, `upgrade` requires `--user` and `--pass` arguments, or a path to the `secrets` file, if the file does not exist already at the expected default path. Please see [elastic-cloud-enterprise.sh upgrade](cloud://reference/cloud-enterprise/ece-installation-script-upgrade.md) for details.
::::
