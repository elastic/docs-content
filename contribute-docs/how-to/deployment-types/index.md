...
Internally, the docs team sometimes refers to everything not hosted on {{ecloud}} as "self-managed," meaning self-managed, ECE, and ECK together. **Do not use this categorization in published documentation.** Readers think about their specific deployment type. Always name the deployment type explicitly in user-facing content.
...
- A **flavor** of the stack — unversioned, continuously delivered, behaves differently from versioned Elasticsearch and Kibana
