...
. *Follow the guided index flow*: Continue with the steps displayed on the UI. This includes <<elasticsearch-get-started-create-index,creating an index>> and using the provided code examples to ingest data.
. *Follow the in-product Getting Started tutorial*: Use the in-product guide to learn how to ingest data and perform searches.
. *Explore on your own*:
- Get your <<elasticsearch-connecting-to-es-serverless-endpoint,connection details>>.
- Choose an <<elasticsearch-ingest-your-data,ingestion option>>.
